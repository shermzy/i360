...
<<<<<<< .mine
	private WritableCellFormat No_Borders, No_Borders_ctrAll,
			No_Borders_ctrAll_Bold, No_Borders_NoBold;
=======
...
>>>>>>> .r1335
	...
	private int SurveyID = 0; // Denise 07/01/2010 store surveyID as global
								// variable for method getHideNA and get
								// NA_Exclude
	...
	public void write(String fileName, int sheetID, int TargetID)
			throws IOException, WriteException, BiffException, Exception {
		System.out.println("FILENAME IS " + fileName + " with sheetID = "
				+ sheetID);
		if (sheetID == 0) {
			String output = server.getReport_Path() + "\\" + fileName;
			outputWorkBook = new File(output);

			inputWorkBook = new File(server.getReport_Path_Template()
					+ "\\HeaderFooter.xls");
		...
		writesheet = workbook.createSheet("" + TargetDetail[0] + ", "
				+ TargetDetail[1], sheetID);

		fontFace = new WritableFont(WritableFont.TIMES, 12,
	...
	public void Header(int SurveyID, int TargetID) throws IOException,
			WriteException, SQLException, Exception {
		...
		label = new Label(2, row_title,
				UnicodeHelper.getUnicodeStringAmp(CompName), No_Borders);
		writesheet.addCell(label);
		...
		label = new Label(2, row_title + 2,
				UnicodeHelper.getUnicodeStringAmp(OrgName), No_Borders);
		writesheet.addCell(label);
		...
		label = new Label(2, row_title + 4,
				UnicodeHelper.getUnicodeStringAmp(SurveyName), No_Borders);
		writesheet.addCell(label);
		...
		label = new Label(2, row_title + 6, TargetDetail[0] + ", "
				+ TargetDetail[1], No_Borders);
		writesheet.addCell(label);
...
=======
		...
		//System.out.println(temp);
		//writesheet.setHeader("", "", "Pacific Century Consulting Pte Ltd.");
		//writesheet.setFooter("Date of printing: " + temp +  "\n" + "Copyright ï¿½3-Sixty Profilerï¿½is a product of Pacific Century Consulting Pte Ltd.", "", "Page &P of &N");
	
>>>>>>> .r1294
	}

	/*
	 * Change : put the self comment to the right most column Reason : Change
	 * format of the report Add by : Denise Add on : 10/12/2009
	 */
<<<<<<< .mine
	public void printRaterHeader(int Result_col, int row_data, int SurveyID,
			int TargetID) throws IOException, WriteException, SQLException,
			Exception {
=======
...
<<<<<<< .mine
			...
			if (RaterCode.equals("SELF")) // store the position of the SELF
											// rater
=======
...
<<<<<<< .mine
			...
			if (xAssID != AssID && !RaterCode.equals("SELF")) // output other
																// raters first
			{
				label = new Label(Result_col, row_data, RaterCode,
						cellBOLD_Border);
=======
			}
...
<<<<<<< .mine
		...
		if (selfPos != -1 && xAssID != selfPos) // if have self comment, output
												// the SELF rater name
		{
...
>>>>>>> .r1294
	...
	public boolean AllRaters(int SurveyID, int TargetID, int PKUser,
			String fileName) throws IOException, WriteException, SQLException,
			Exception {
		boolean IsNull = false;
		this.SurveyID = SurveyID; // Denise 07/01/2010
		...
		for (int sheetID = 0; sheetID < targets.size(); sheetID++) {
			...
			// Denise 30/12/2009 find the position of SELF in the
			// targetAssignemnt list
			int y = 0;
			while (y < v.size()) {
			...
			for (int j = 0; j < vRating.size(); j++) {
				...
				if (xRatingTaskID != RatingTaskID) {
					 ...
					 * Level---------------------------
					 */
					label = new Label(col, row_data, "Competency",
							cellBOLD_Border);
					if (server.LangVer == 2)
						label = new Label(col, row_data, "Kompetensi",
								cellBOLD_Border);
					writesheet.addCell(label);
					...
					if (LevelOfSurvey == 0) {
						Result_col = 1;
						/*
						 * Print the 1st row of the table - the list of rater for that person
						 */
						printRaterHeader(Result_col, row_data, SurveyID,
								TargetID);
						...
						int xCompID = 0;
						/*
						 * Print the 1st column of the table -  the competencies
						 */
						for (int k = 0; k < vComp.size(); k++) {
							voCompetency voComp = (voCompetency) vComp
									.elementAt(k);
							...
							if (xCompID != CompID) {
								label = new Label(col, row,
										UnicodeHelper
												.getUnicodeStringAmp(CompName),
										bordersData2);
								writesheet.addCell(label);
								
								// Edit by Denise 12/12/2009 use the new
								// printRaterResult_KB function to change the
								// SELF comment to the right most column
								// Edit by Denise 30/12/2009 use the old
								// printRaterResult_KB function since the SELF
								// comment has been moved to the end in the
								// arr_AssID
								printRaterResult_Comp(Result_col, row,
										arr_AssID, CompID, RatingTaskID); // can
																			// use
																			// this
																			// one
																			// if
								...
								// printRaterResult_Comp(Result_col, row,
								// arr_AssID,raterCode, CompID, RatingTaskID);
					...
					if (LevelOfSurvey == 1) {
						...
						printRaterHeader(Result_col, row_data, SurveyID,
								TargetID);
						label = new Label(col + 1, row_data,
								"Key Behaviour Statement", cellBOLD_Border);
						if (server.LangVer == 2)
							label = new Label(col + 1, row_data,
									"Pernyataan Perilaku Kunci",
									cellBOLD_Border);
						writesheet.addCell(label);
						...
						/*
						 * resultSQL =
						 * "SELECT * FROM tblSurveyBehaviour INNER JOIN";
						 * resultSQL = resultSQL +
						 * " Competency ON tblSurveyBehaviour.CompetencyID = Competency.PKCompetency AND"
						 * ; resultSQL = resultSQL +
						 * " tblSurveyBehaviour.CompetencyID = Competency.PKCompetency INNER JOIN"
						 * ; resultSQL = resultSQL +
						 * " KeyBehaviour ON tblSurveyBehaviour.KeyBehaviourID = KeyBehaviour.PKKeyBehaviour AND"
						 * ; resultSQL = resultSQL +
						 * " Competency.PKCompetency = KeyBehaviour.FKCompetency"
						 * ; resultSQL = resultSQL +
						 * " WHERE (tblSurveyBehaviour.SurveyID = "
						 * +SurveyID+" )";
						 */
						...
						int xKeyID = 0;
						/*
						 * This part is not used in generation of SPF report
						 */
						...
						for (int k = 0; k < vKB.size(); k++) {
							...
							if (xKeyID != KeyID) {
								label = new Label(col, row + 1, " ",
										bordersData2);
								writesheet.addCell(label);
								label = new Label(col + 1, row + 1,
										UnicodeHelper
												.getUnicodeStringAmp(KeyName),
										bordersData2);
								writesheet.addCell(label);
							...
							// Edit by Denise 10/12/2009 use the new
							// printRaterResult_KB function to change the SELF
							// comment to the right most column
							// Edit by Denise 30/12/2009 use the old
							// printRaterResult_KB function since the SELF
							// comment has been moved to the end in the
							// arr_AssID
							printRaterResult_KB(Result_col, row, arr_AssID,
									KeyID, RatingTaskID); // can use this one if
															// want to output
															// the assignments
							// according to the order in the database
							// printRaterResult_KB(Result_col, row,
							// arr_AssID,raterCode, KeyID, RatingTaskID);
							row = row + 2;
						}
					}
			...
			/*
<<<<<<< .mine
			 * Change : Put checking whether the comment is included in report
			 * or not Reason : no checking before Add by : Johanes Add on :
			 * 26/10/2009
			 */
...
>>>>>>> .r1294
			...
			ev.addRecord("Insert", itemName, "List Of Rater Input for "
					+ targetdetail[0] + ", " + targetdetail[1]
					+ "(Target) for Survey " + OldName, sDetail[2],
	 ...
	 * @throws Exception
	 */
	public void printRaterResult_KB(int Result_col, int row, int[] arr_AssID,
			int KeyID, int RatingTaskID) throws IOException, WriteException,
			SQLException, Exception {
		// Denise 06/01/2010 to check the hideNA and NA excluded option of the
		// survey
		Create_Edit_Survey CE = new Create_Edit_Survey();
		boolean hideZero = CE.getHideNAOption(SurveyID) == 1
				|| CE.getNA_Included(SurveyID) == 0;

		for (int d = 0; d < arr_AssID.length; d++) {
			if (arr_AssID[d] != 0) {
				AssignTarget_Rater ATR = new AssignTarget_Rater();

				double result = ATR.getKBResult(arr_AssID[d], KeyID,
						RatingTaskID);

				double SurvResult = Math.round(result * 100.0) / 100.0;
				if (result != -1
						&& !(hideZero && (int) Math.round(SurvResult) == 0)) // Denise
																				// 06/01/2010
																				// ,
																				// if
																				// the
				...
				{ // output empty cell
					...
					Number num = new Number(Result_col, row + 1, SurvResult,
							bordersData1);
	...
	/*
	 * function printRaterResult_KB: print the result of each assignment. Use
	 * the arr_raterCode to store the rater of each assignment Add by : Denise
	 * Add on : 10/12/2009
	 */
	/*
	 * public void printRaterResult_KB(int Result_col, int row, int []
	 * arr_AssID,String[] arr_raterCode, int KeyID, int RatingTaskID) throws
	 * IOException, WriteException, SQLException, Exception { int selfPos = -1;
	 * int d = 0;
	 * 
	 * AssignTarget_Rater ATR = new AssignTarget_Rater(); while ( d<
	 * arr_AssID.length && arr_raterCode[d]!=null) { if
	 * (arr_raterCode[d].equals("SELF")) //store the position of the SELF
	 * assignment { selfPos = d; d++; continue; }
	 * 
	 * //output other assignments first
	 * printRaterResult_Cell(Result_col,row,arr_AssID[d], KeyID, RatingTaskID,
	 * arr_raterCode[d]); Result_col = Result_col+1; d++; }
	 * 
	 * if (selfPos !=-1) //if have the SELF assignment, output the result { d =
	 * selfPos; printRaterResult_Cell(Result_col,row,arr_AssID[d], KeyID,
	 * RatingTaskID, arr_raterCode[d]); } }
	 * 
	 * /* function printRaterResult_Cell: print the result of assignment AssID
	 * at row row and column Result_col Add by : Denise Add on : 10/12/2009
	 */
	/*
	 * private void printRaterResult_Cell(int Result_col, int row, int AssID,
	 * int KeyID, int RatingTaskID, String name) throws IOException,
	 * WriteException, SQLException, Exception { AssignTarget_Rater ATR = new
	 * AssignTarget_Rater(); double result = ATR.getKBResult(AssID, KeyID,
	 * RatingTaskID); if(result != -1) { double SurvResult = Math.round(result *
	 * 100.0) / 100.0;
	 * 
	 * label= new Label(Result_col, row, " " , bordersData2);
	 * writesheet.addCell(label);
	 ...
	 * writesheet.addCell(num); } else { label= new Label(Result_col, row, " " +
	 * name , bordersData2); writesheet.addCell(label);
	 * 
	 * label= new Label(Result_col, row+1, " " +name, bordersData2);
	 * writesheet.addCell(label); } }
	 */
	public void printRaterResult_Comp(int Result_col, int row, int[] arr_AssID,
			int CompID, int RatingTaskID) throws IOException, WriteException,
			SQLException, Exception {
		...
		for (int d = 0; d < arr_AssID.length; d++) {
			if (arr_AssID[d] != 0) {
				...
				double result = ATR.getCompResult(arr_AssID[d], CompID,
						RatingTaskID);

				double SurvResult = Math.round(result * 100.0)/100;
				...
				{ // output empty cell
					...
					Number num = new Number(Result_col, row, SurvResult,
							bordersData1);
					writesheet.addCell(num);
				} else {
					label = new Label(Result_col, row, " ", bordersData2);
...
<<<<<<< .mine
	 */
	/*
	 * public void printRaterResult_Comp(int Result_col, int row, int []
	 * arr_AssID, String[] arr_raterCode, int CompID, int RatingTaskID) throws
	 * IOException, WriteException, SQLException, Exception { int d = 0; int
	 * selfPos = -1;
	 * 
	 * while ( d< arr_AssID.length && arr_raterCode[d]!=null) { if
	 * (arr_raterCode[d].equals("SELF")) //store position of the SELF assignment
	 * { selfPos = d; d++; continue; }
	 * 
	 * //output other rating result first AssignTarget_Rater ATR = new
	 * AssignTarget_Rater();
	 * 
	 * double result = ATR.getCompResult(arr_AssID[d], CompID, RatingTaskID);
	 * 
	 * if(result !=-1) { double SurvResult = Math.round(result * 100.0) / 100.0;
	 * 
	 * Number num = new Number(Result_col, row, SurvResult, bordersData1);
	 * writesheet.addCell(num); } else { label= new Label(Result_col, row, " ",
	 * bordersData2); writesheet.addCell(label); }
	 * 
	 * Result_col = Result_col+1; d++; }
	 * 
	 * if (selfPos !=-1) //if have SELF assignment, output self rating { d =
	 * selfPos; AssignTarget_Rater ATR = new AssignTarget_Rater();
	 * 
	 * double result = ATR.getCompResult(arr_AssID[d], CompID, RatingTaskID);
	 * 
	 * if(result !=-1) { double SurvResult = Math.round(result * 100.0) / 100.0;
	 ...
	 * Result_col = Result_col+1; } }
	 */
	public void printRaterComment(int r, int surveyID, int[] arr_AssID,
			int TargetID) throws IOException, WriteException, SQLException,
			Exception {
		// Added by Ha 13/06/08 to not print out the comment when survey does
		// not have comment
=======
	 */	
			...
			if (arr_AssID[d] != 0) {
				...
				for (int i = 0; i < v.size(); i++) {
					...
					if (RaterCode != null) {
						...
						if (selfIncluded == 1 || !RaterCode.equals("SELF")
								&& included == 1) {
							r++;
							label = new Label(0, r++, "Narrative Comments by "
									+ RaterCode, No_Borders);
							if (server.LangVer == 2)
								label = new Label(0, r++,
										"Komentar Naratif oleh " + RaterCode,
										No_Borders);
							writesheet.addCell(label);
<<<<<<< .mine

							label = new Label(0, r, "Competency",
									cellBOLD_Border);
							if (server.LangVer == 2)
								label = new Label(0, r, "Kompetensi",
										cellBOLD_Border);
							writesheet.addCell(label);
							label = new Label(1, r, "Narrative Comments",
									cellBOLD_Border);
							if (server.LangVer == 2)
								label = new Label(1, r, "Komentar Naratif",
										cellBOLD_Border);
							writesheet.addCell(label);
...
>>>>>>> .r1335
								voCompetency voComp = (voCompetency) compComment
										.elementAt(l);
...
<<<<<<< .mine
								...
								Vector vComment = S
										.getComment(arr_AssID[d], id);
								for (int m = 0; m < vComment.size(); m++) {
									String sComment = (String) vComment
											.elementAt(m);
									if (start != 0) {
										label = new Label(1, r, "",
												bordersData2);
