...
{ 	
	...
	private Vector vGapSorted; // this is to store the gap of each competency so
								// does not need to reopen another resultset
	private Vector vGapUnsorted;
	private Vector vCompID;
	private Vector vCompName;
	private Vector vCPValues; // add to store CP values of each competency for
	private Vector oriRaterCodeList = new Vector();							// sorting , Mark Oei 16 April 2010
	private Vector newRaterCodeList = new Vector();							// sorting , Mark Oei 16 April 2010
	...
	private int iCancel = 0; // If user cancelled the printing process. 0=Not
	private String newGroupName;		// cancelled, 1=Cancelled
	...
	private XSpreadsheet xSpreadsheet5 = null;// used for item frequency
												// spreadsheet
	...
	private int iReportType; // 1=Simplified Report "No Competencies charts",
								// 2=Standard Report
	private int iNoCPR = 1; // 0=CPR is chosen for survey, 1=No CPR chosen for
							// survey
	...
	private int breakCPR = 0; // 0=do not break, 1 breakdown CPR

	private int lastPageRowCount = 0; // For keeping a record of the last page
										// row count
	private boolean isGroupCPLine = false; // For displaying/not displaying
											// Group CP Line
	...
	private int format = 0; // To track the format to use
	...
	/**
	 * To retrieve a list of clusters in the survey which contains comments by raters
	 * @author Albert
	 **/
	public Vector ClusterListHaveComments() throws SQLException 
	{
		...
		query = query + "INNER JOIN tblSurveyCompetency ON (tblSurveyCompetency.CompetencyID = Competency.PKCompetency AND tblAssignment.SurveyID = tblSurveyCompetency.SurveyID) ";
		query = query + "INNER JOIN Cluster ON Cluster.PKCluster = tblSurveyCompetency.ClusterID ";
		...
		try{
			...
			System.out.println("IndividualReport.java - ClusterListHaveComments - "+ex.getMessage());
		}
		finally{
			ConnectionBean.closeRset(rs); //Close ResultSet
			ConnectionBean.closeStmt(st); //Close statement
	...
	/**
	 * To retrieve a list of competencies in the survey which belong to a particular cluster which contains comments by raters
	 * @author Albert
	 **/
	public Vector ClusterCompListHaveComments(int clusterID) throws SQLException 
	{
		...
		try{
			...
			System.out.println("IndividualReport.java - ClusterCompListHaveComments - "+ex.getMessage());
		}
		finally{
			ConnectionBean.closeRset(rs); //Close ResultSet
			ConnectionBean.closeStmt(st); //Close statement
	...
	{	
		...
		/* query the database to get the number of rated questionnaires received by the "Target"(targetID) on a particular
		 * "survey"(surveyID) and on the type of questionnaire answered(1-superior,2-self,4-subordinate)
		 */
		String query = "SELECT COUNT(DISTINCT type) from tblavgMean where surveyID="+ surveyID + " and targetLoginId = " + targetID;
	...
	public Vector<String> otherRT(int surveyID){
		Vector<String> v = new Vector<String>();
		String query = "SELECT (tblRatingTask.RatingCode) AS TotalRT FROM tblSurveyRating INNER JOIN " +
				"tblRatingTask ON tblSurveyRating.RatingTaskID = tblRatingTask.RatingTaskID WHERE " +
				"tblSurveyRating.SurveyID = "+ surveyID +" and tblRatingTask.RatingCode != 'CP'";
		...
		try{
			...
			if(rs.next())
				v.add(rs.getString("TotalRT")+"(All)");
	...
	public Vector CPCPR(String RTCode) throws SQLException 
	{
		...
		if (reliabilityCheck == 0) {
			query = "SELECT distinct tblTrimmedMean.CompetencyID, Competency.CompetencyName, tblTrimmedMean.TrimmedMean as Result ";
			...
			if (surveyLevel == 0) {
				query = "SELECT distinct tblAvgMean.CompetencyID, Competency.CompetencyName, tblAvgMean.AvgMean as Result ";
				query = query + "FROM tblAvgMean INNER JOIN tblRatingTask ON ";
				query = query
				...
				query = query + "tblAvgMean.TargetLoginID = " + targetID
						+ " AND tblAvgMean.Type =" + cpAll + " AND tblassignment.round = "+ round + " and ";
				query = query + "tblRatingTask.RatingCode = '" + RTCode
						+ "' ORDER BY Competency.CompetencyName";
			} else {
				query = "SELECT distinct tblAvgMean.CompetencyID, Competency.CompetencyName, ";
				query = query
		...
		try {
			...
			while (rs.next()) {
				...
				arr[2] = df.format(Math.round(Double.parseDouble(rs.getString(3))*100)/100.00);
				v.add(arr);
			}
		} catch (Exception ex) {
			System.out.println("IndividualReport.java - CPCPR - "
					+ ex.getMessage());
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
			ConnectionBean.closeStmt(st); // Close statement
	...
	public Vector getOtherTargetResults(int ID, int compID) throws SQLException 
	{
		...
		if (reliabilityCheck == 0) {
			...
			if (surveyLevel == 0) {
				query = query
						+ "SELECT distinct tblAvgMean.CompetencyID, Competency.CompetencyName, tblAvgMean.AvgMean as Result ";
				query = query + "FROM tblAvgMean INNER JOIN tblRatingTask ON ";
				query = query
						+ "tblAvgMean.RatingTaskID = tblRatingTask.RatingTaskID ";
				query = query + "INNER JOIN Competency ON ";
				...
				query = query
						+ "SELECT distinct tblAvgMean.CompetencyID, Competency.CompetencyName, ";
				query = query
						+ "CAST(AVG(tblAvgMean.AvgMean) AS numeric(38, 2)) AS Result ";
				query = query + "FROM tblRatingTask INNER JOIN tblAvgMean ON ";
				query = query
		...
		try {
			...
			System.out
					.println("IndividualReport.java - getOtherTargetResults - "
							+ ex.getMessage());
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
			ConnectionBean.closeStmt(st); // Close statement
	...
	public void InitializeExcel(String sSavedFileName, String sTemplateName)
		...
		try{ 
			...
			errors.add("Missing Sheet 'Individual Report' in template file");
			e.printStackTrace();
		}
		try{
			xSpreadsheet2 = OO.getSheet(xDoc, "Sheet2");
		}catch(Exception e){
			errors.add("Missing Sheet 'Sheet 2' in template file");
			e.printStackTrace();
		}
		...
		try{
			...
			errors.add("Missing Sheet 'Item Frequency' in template file");
			e.printStackTrace();
		}
		try{
			xSpreadsheet4 = OO.getSheet(xDoc, "Ranking Table");
		}catch(Exception e){
			errors.add("Missing Sheet 'Cluster Ranking Table' in template file");
			e.printStackTrace();
		}
		try{
			xSpreadsheet5 = OO.getSheet(xDoc, "Group Ranking Table");
		}catch(Exception e){
			errors.add("Missing Sheet 'Group Ranking table' in template file");
			e.printStackTrace();
		}
	...
	public void InsertIndividualProfileLegend(char num, String title, String desc, int row, int col) throws Exception{
		...
		OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, desc),
				row + 1, column + 1);
	...
	public void InsertProfileLegend(){
		...
		try {
			...
			InsertIndividualProfileLegend(i,
					"Current Proficiency - All (CP - ALL)", desc, row + j,
					column);
			OO.setRowHeight(xSpreadsheet, row + j + 1, startColumn + 1,
					ROWHEIGHT * OO.countTotalRow(desc, 90));
			i++;
			...
			InsertIndividualProfileLegend(i, "Level Of Agreement", desc, row
					+ j, column);
			OO.setRowHeight(xSpreadsheet, row + j + 1, startColumn + 1,
					ROWHEIGHT * OO.countTotalRow(desc, 90));
			i++;
			j += 3;
			OO.deleteRows(xSpreadsheet, startColumn, endColumn, row + j + 1,
					row + j + 6, 5, 1);
	...
	public double getGroupCPForOneCompetency(String compName) throws SQLException 
	{
		...
		query = "SELECT Competency.PKCompetency AS CompetencyID, Competency.CompetencyName, "
				...
				+ "tblAvgMean.RatingTaskID = tblRatingTask.RatingTaskID inner join tblassignment on tblassignment.targetLoginID =tblavgmean.targetloginid"
		        + " INNER JOIN [User] ON [User].PKUser = tblAvgMean.TargetLoginID "
				...
				+ " AND tblAvgMean.Type = "
				+ cpAll
				+ " AND tblRatingTask.RatingCode = 'CP' and tblassignment.round=" + round + " AND "
	...
	public Vector getGroupCP() throws SQLException 
	{
		...
		query = "SELECT Competency.PKCompetency AS CompetencyID, Competency.CompetencyName, " 
				...
				+ "[User].PKUser = tblAvgMean.TargetLoginID inner join tblassignment on tblassignment.targetLoginID = tblavgmean.targetLoginID "
				+ "WHERE tblAvgMean.SurveyID = " + surveyID + " AND tblAvgMean.Type = " + type + " and tblassignment.round =" + round+" AND tblRatingTask.RatingCode = 'CP' AND " 
				+ "tblAvgMean.TargetLoginID IN "
				+ "(SELECT TargetLoginID FROM tblAssignment INNER JOIN [USER] ON [USER].PKUser = tblAssignment.TargetLoginID " 
	...
	public void InsertGapTitleCluster(int surveyID) throws SQLException, IOException, Exception
	{
		...
		try {
			...
			String info = "";
				...
				info = "The table below shows the competency scores of the Feedback Recipient in accordance to the clusters.";
			...
			OO.setFontBold(xSpreadsheet, startColumn, endColumn, row - 1,
					row - 1);
			...
			System.err
					.println("IndividualReport.java - InsertGapTitleCluster - "
							+ E);
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
			ConnectionBean.closeStmt(st); // Close statement
	...
	public void InsertGapTitle(int surveyID) throws SQLException, IOException, Exception
	{
		...
		try {
			...
			System.out.println("scol "+startColumn);
			System.out.println("ecol "+endColumn);
			System.out.println("xspreadsheet"+xSpreadsheet);
			//OO.insertRows(xSpreadsheet, startColumn, endColumn, row + 3,
				//	row + 3, 1, 1);
	...
	public void InsertGapCluster() throws SQLException, IOException, Exception{
		...
		if (iNoCPR == 0) // If CPR is chosen in this survey
		{
			...
			row++;
			OO.insertRows(xSpreadsheet, startColumn, endColumn, row, row + 2,
					2, 1);
			...
			for (int i = 0; i < v.size(); i++) {
				...
				OO.setCellAllignment(xSpreadsheet, cpCol, gcpCol, row, row, 1,
						2);
					...
					if (compList.contains(compName)) {
						...
						if (isGroupCPLine) {
							gcpValue = getGroupCPForOneCompetency(compName
									.trim());
							OO.insertNumeric(xSpreadsheet, gcpValue, row,
									gcpCol);
						}

						OO.setFontSize(xSpreadsheet, cpCol, gcpCol, row, row,
								12);
						row++;
					}
				}
				...
				// Mark Oei 25 Mar 2010
				OO.setTableBorder(xSpreadsheet, cpCol, endColumn - 1,
						startBorder, endBorder, true, false, true, true, false,
						false);
			...
			OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, title),
					row - 4, c);
			OO.setFontSize(xSpreadsheet, startColumn, endColumn, row - 4,
					row - 4, 16);
			OO.setFontBold(xSpreadsheet, startColumn, endColumn, row - 4,
					row - 4);
			...
			for (int i = 0; i < v.size(); i++) {
				...
				OO.setCellAllignment(xSpreadsheet, cpCol - 1, gcpCol, row, row,
						1, 2);
				OO.setBGColor(xSpreadsheet, startColumn, endColumn - 1, row,
						row, BGCOLORCLUSTER);
				...
				for (int k = 0; k < compList.length; k++) {
					...
					if (compList[k] != null) {
						...
						OO.setFontSize(xSpreadsheet, cpCol - 1, gcpCol, row,
								row, 12);
						totalCP += cpValues;
						totalGCP += gcpValue;
						row++;
				...
				OO.setCellAllignment(xSpreadsheet, 10, 11, startBorder - 1,
						row, 1, 2);
				row++;

				endBorder = row;
				OO.setTableBorder(xSpreadsheet, startColumn, endColumn - 1,
						startBorder, endBorder - 1, false, false, true, true,
						true, true);
				// Add border lines from cpCol to gcpCol
				// Mark Oei 25 Mar 2010
				OO.setTableBorder(xSpreadsheet, cpCol, endColumn - 1,
						startBorder, endBorder - 1, true, false, true, true,
						false, false);
	...
	public void InsertGap() throws SQLException, IOException, Exception 
	{
		...
		if (iNoCPR == 0) // If CPR is chosen in this survey
		{
			...
			// if (ST.LangVer == 2)
			// title = "KOMPETENSI";
			OO.insertRows(xSpreadsheet, startColumn, endColumn, row - 1,
					row + 1, 2, 1);
			OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, title),
					row, c);
			OO.setFontBold(xSpreadsheet, startColumn, endColumn, row, row);
			...
			OO.setFontBold(xSpreadsheet, startColumn, endColumn, row, row);
			OO.setFontSize(xSpreadsheet, cpCol - 1, gcpCol, row, row, 12);
			OO.setCellAllignment(xSpreadsheet, cpCol - 1, gcpCol, row, row, 1,
					2);
			OO.setBGColor(xSpreadsheet, startColumn, endColumn - 1, row, row,
					BGCOLOR);
			row++;
			...
			for (int i = 0; i < vCPValues.size(); i++) {
				...
				if (cpValues >= high) {
					...
					OO.setFontSize(xSpreadsheet, cpCol - 1, gcpCol, row, row,
							12);
	...
	private int[] prepareCells(XSpreadsheet xSpreadsheet, int startingRow, int totalRater) throws Exception {
		...
		if (totalRater == 8) {
			...
			result[6] = startingRow + 11;
			result[7] = startingRow + 12;
		}else if (totalRater == 7) {

			result[0] = startingRow + 2;
			result[1] = startingRow + 3;
			OO.mergeCells(xSpreadsheet,0,0,startingRow+3,startingRow+4);
			OO.setCellAllignment(xSpreadsheet,0,0,startingRow+3,startingRow+4,2,2);
			result[2] = startingRow + 5;
			result[3] = startingRow + 6;
			OO.mergeCells(xSpreadsheet,0,0,startingRow+6,startingRow+7);
			OO.setCellAllignment(xSpreadsheet,0,0,startingRow+6,startingRow+7,2,2);
			result[4] = startingRow + 8;
			result[5] = startingRow + 9;
			result[6] = startingRow + 10;
			OO.mergeCells(xSpreadsheet,0,0,startingRow+10,startingRow+11);
			OO.setCellAllignment(xSpreadsheet,0,0,startingRow+10,startingRow+11,2,2);
			result[7] = startingRow + 12;
		}else if (totalRater == 7) {
			result[0] = startingRow + 2;
			result[1] = startingRow + 3;
	 ...
	 * 
	 */
	private int[] prepareCellsBreakCPR(XSpreadsheet xSpreadsheet, int startingRow, int totalRater) throws Exception {
		...
		if (totalRater == 12) {
			...
			OO.setCellAllignment(xSpreadsheet,0,0,startingRow+7,startingRow+8,2,2);
	...
	// only can search for Cp, or other type like CPR
	//type 0 is CP, type 1 means others
	public int getArrIndex(String info, String[] ratings, int type){
		if(type != 0){
			for(int i = 0; i < ratings.length; i++){
				if(ratings[i].contains(info) && ratings[i].contains("CPR"))
					return i;
			}
		}else{
			for(int i = 0; i < ratings.length; i++){
				if(ratings[i].contains(info) && ratings[i].contains("CP") && !(ratings[i].contains("CPR")))
					return i;
			}
		}
		return -1;
	...
	public String mapRaterCodeToDesc(String code, String prefix){
	    if(code.contains("ADD")){
	    	return (prefix+"(Additional)");
	    }else if(code.contains("IDR")){
	    	return(prefix+"(Indirect)");
	    }else if(code.contains("SEL")){
	    	return(prefix+"(Self)");
	    }else if(code.contains("SUP")){
	    	return(prefix+"(Superior)");
	    }else if(code.contains("OTH")){
	    	return(prefix+"(Others)");
	    }else if(code.contains("PEE")){
	    	return(prefix+"(Peers)");
	    }else if(code.contains("SUB")){
	    	return(prefix+"(Subordinates)");
	    }else if(code.contains("DIR")){
	    	return(prefix+"(Direct)");
	    }else if(code.contains("BOS")){
	    	return(prefix+"(BOSS)");
	    }
	    return null;
	}
	public Vector<String> totalRaterType(int surveyID, Vector combinedGroup, String newGroupName, String prefix){
		  Vector<String> totalRater = new Vector<String>();
		  totalRater.add(prefix + "(All)");
		 String query = "select distinct substring(ratercode,0,4) as ratercode, Num from tblAssignment inner join" +
		  		" tblRelationSequence on substring(relation,0,4) = substring(ratercode,0,4)where surveyID = " + surveyID +
		  		"order by Num";
		  ...
		  try{
		   ...
		   while(rs.next()){
		    String code = rs.getString("ratercode");
		    
		    totalRater.add(mapRaterCodeToDesc(code, prefix));
		   }
		   
		   if(combinedGroup != null && combinedGroup.size()>1 && newGroupName != null){
			   for(int i = 0; i < combinedGroup.size(); i++){
				   System.err.println("delete " + (String)combinedGroup.get(i));
				   int index = totalRater.indexOf(mapRaterCodeToDesc((String)combinedGroup.get(i), prefix));
				   totalRater.remove(index);
			   }
			   totalRater.add(prefix+"("+newGroupName+")");
		   }

		  }catch(Exception ex){
		   System.err.println("IndividualReport.java - totalRaterType ");
		   ex.printStackTrace();
		  }
		  finally{
		   ConnectionBean.closeRset(rs); //Close ResultSet
	...
	public void InsertClusterCompetency(int reportType, int SurveyID,
			int breakCPR, Vector combinedRateGroup, String newGroupName) throws SQLException, Exception {
		...
		if (breakCPR == 0) {// without breaking CPR
			...
			Vector<String> allRaterType = totalRaterType(surveyID, combinedRateGroup, newGroupName, "CP");
			int totalType = allRaterType.size();

			// used variable totalType to determine the initialization of array
			// size (16 Dec 2009 Qiao Li)
			String[] Rating = new String[totalType + totalOtherRT + 1]; 
			for(int n = 0; n < totalType; n++){
				Rating[n] = allRaterType.get(n);
			}
			Rating[(totalType+1)] = "BOSS";
			double[] Result = new double[totalType + totalOtherRT];
			int[] totalRater = new int[totalType + totalOtherRT]; 
			...
			if (surveyLevel == 0){  // Competency Level Survey
				...
				for (int m = 0; m < vClust.size(); m++) {
					...
					for (int i = 0; i < vComp.size(); i++) {
						...
						// Aug 2011
						OO.insertString(xSpreadsheet,getTranslatedCompetency(
						...
						OO.setFontType(xSpreadsheet, 0, 11, row, row,
								"Times new Roman");
						OO.mergeCells(xSpreadsheet, startColumn, endColumn,
								row, row);
						...
						for (int j = 0; j < RT.size(); j++) {
							...
							if (RTCode.equals("CP")) {
								 ...
								 */
								totalPeer = totalRater(RTID, compID, -1,
										"PEER%", surveyLevel);
								totalDirect = totalRater(RTID, compID, -1,
										"DIR%", surveyLevel);
								...
								totalAll = totalSup + totalPeer + totalDirect
										+ totalIndirect + totalSub + totalOth + totalAdd;
								System.out.println(" Line 5998 - Peer : " + totalPeer + " Sub : " + totalSub + " totalAdd : " + totalAdd + " Total Boss : " + totalBOSS);
								boolean weightedAvg = false;

								// check if combination happened
								boolean combineOccur = false;
								int numofAffectedRater = 0;
								Vector<Integer> affectedGroup = new Vector<Integer>();
								
								for (int k = 0; k < result.size(); k++) {
									String[] arrOther = (String[]) result
											.elementAt(k);

									int type = Integer.parseInt(arrOther[1]); //position 0 is competency/kb, position 1 is type, 2 is result
									String t = "";
									...
									case 1:
										...
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalAll;
											Result[getArrIndex(t, Rating,0)] = Double
													.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalAll;
											affectedGroup.add(type);
										}
										break;
									case 2:
										...
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalSup;
											Result[getArrIndex(t, Rating,0)] = Double
													.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalSup;
											affectedGroup.add(type);
										}
										break;
									// added back the others for type
									...
									case 3:
										...
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalOth;
											Result[getArrIndex(t, Rating,0)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalOth;
											affectedGroup.add(type);
										}
										break;
									case 4:
										...
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalSelf;
											Result[getArrIndex(t, Rating,0)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalSelf;
											affectedGroup.add(type);
										}
										break;

									// Added case 5 and 6 to cater to splitting
									// of Peers and Subordinates by Desmond 21
									// Oct 09
									case 5:
										t = "Subordinates";
										arrN[iN] = totalSub;
										iN++;
										try{
										totalRater[getArrIndex(t,Rating,0)] = totalSub;
										Result[getArrIndex(t, Rating,0)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalSub;
											affectedGroup.add(type);
										}
										break;
									case 6:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalPeer;
											Result[getArrIndex(t, Rating,0)] = Double
													.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalPeer;
											affectedGroup.add(type);
										}
										break;
									...
									case 7:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalAdd;
											Result[getArrIndex(t, Rating,0)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalAdd;
											affectedGroup.add(type);
										}
										break;
									...
									case 8:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalDirect;
											Result[getArrIndex(t, Rating,0)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalDirect;
											affectedGroup.add(type);
										}
										break;
									...
									case 9:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalIndirect;
											Result[getArrIndex(t, Rating,0)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalIndirect;
											affectedGroup.add(type);
										}
										break;
								...
								if(combineOccur){
									totalRater[getArrIndex(newGroupName,Rating,0)] = numofAffectedRater;
									Result[getArrIndex(newGroupName,Rating,0)] = 
											C.CombineGroupMeanResult(surveyID,targetID,affectedGroup, compID, KBID, false);
								}
								if (weightedAvg == true) {
									//Rating[0] = RTCode + "(All)";
									...
									Result[getArrIndex("All", Rating,0)] = weightedAvgScore;
								}
							} // end of if CP
							else if(RTCode.equals("CPR") || RTCode.equals("FPR")){
								totalSup = totalSup(RTID, compID);
								totalSelf = totalSelf(RTID, compID);
								totalPeer = totalRater(RTID, compID, -1,
										"PEER%", surveyLevel);
								totalDirect = totalRater(RTID, compID, -1,
										"DIR%", surveyLevel);
								...
								// and Subordinates by Desmond 21 Oct 09
								totalAll = totalSup + totalPeer + totalDirect
										+ totalIndirect + totalSub + totalAdd + totalOth;
								...
								for (int k = 0; k < result.size(); k++) {
									...
									case 1:
										...
										// set the valur of total all here
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalAll;
											Result[getArrIndex(t, Rating,1)] = Double
													.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalAll;
									...
									case 2:
										...
										// set the valur of total sup here
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalSup;
											Result[getArrIndex(t, Rating,1)] = Double
													.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 3:
										...
										// set the valur of total others here
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalOth;
											Result[getArrIndex(t, Rating,1)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 4:
										...
										// set the valur of total self here
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalSelf;
											Result[getArrIndex(t, Rating,1)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 5:
										...
										iN++;
										try{
										totalRater[getArrIndex(t,Rating,1)] = totalSub;
										Result[getArrIndex(t, Rating,1)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 6:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalPeer;
											Result[getArrIndex(t, Rating,1)] = Double
													.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 7:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalAdd;
											Result[getArrIndex(t, Rating,1)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 8:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalDirect;
											Result[getArrIndex(t, Rating,1)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
									...
									case 9:
										...
										iN++;
										try{
											totalRater[getArrIndex(t,Rating,1)] = totalIndirect;
											Result[getArrIndex(t, Rating,1)] = Double
												.parseDouble(arrOther[2]);
										}catch(Exception e){
											combineOccur = true;
								...
								if(combineOccur){
									totalRater[getArrIndex(newGroupName,Rating,1)] = numofAffectedRater;
									Result[getArrIndex(newGroupName,Rating,1)] = 
											C.CombineGroupMeanResult(surveyID,targetID,affectedGroup, compID, KBID, true);
								}
							}
						}
						...
						OO.setTableBorder(xSpreadsheet, startColumn, endColumn,
								startRow, startRow + 1, false, false, true,
								true, true, true);

						// total sup n others
						OO.setTableBorder(xSpreadsheet, startColumn,
								startColumn, startRow + 2, endRow, false,
								false, true, true, true, true);
						// chart
						OO.setTableBorder(xSpreadsheet, startColumn + 1, 8,
								startRow + 2, endRow, false, false, true, true,
								true, true);
						...
						if (blnSupIncluded) {// added by Ping Yang on 11/08/08,
							...
							for (int j = 0; j < supComments.size(); j++) {
								...
								OO.setFontSize(xSpreadsheet, 9, 11, row, row,
										12);
								OO.setFontType(xSpreadsheet, 0, 11, row, row,
										"Times new Roman");
								...
								if (!comment.trim().equals("")) {// Added by
									...
									OO.insertString(
											...
											row, column + 1);
									OO.justify(xSpreadsheet, column + 1, column + 1, row, row);
									...
									OO.setCellAllignment(xSpreadsheet,
											startColumn, startColumn, row, row,
											2, 1);
									row++;
						...
						for (int k = 0; k < peerComments.size(); k++) {
							...
							if (start == 0) {
								// Allow dynamic translation, Chun Yeong 1 Aug
								// 2011
								OO.setFontSize(xSpreadsheet, 0, 11, row, row,
										12);
								OO.setFontType(xSpreadsheet, 0, 11, row, row,
										"Times new Roman");
								OO.insertString(
						...
						boolean blnDirCommentExist = false; // Added to get rid
															// of extra '-'s
						for (int k = 0; k < dirComments.size(); k++) {
							String[] arr = (String[]) dirComments.elementAt(k);
							String comment = arr[1];
							...
							if (start == 0) {
								...
								OO.setFontType(xSpreadsheet, 0, 11, row, row,
										"Times new Roman");
								OO.insertString(xSpreadsheet, trans.tslt(
										templateLanguage, "Direct Report(s)"),
										row, column + 1);
								OO.setFontBold(xSpreadsheet, startColumn,
										endColumn, row, row);
								OO.setFontItalic(xSpreadsheet, startColumn,
						...
						boolean blnIndirCommentExist = false; // Added to get
																// rid of extra
																// '-'s
							...
							if (selfComments != null) {
								...
								for (int j = 0; j < selfComments.size(); j++) {
									...
									if (start == 0) {
										...
										// 1 Aug 2011
										OO.setFontSize(xSpreadsheet, 0, 11,
												row, row, 12);
										OO.setFontType(xSpreadsheet, 0, 11,
												row, row, "Times new Roman");
										OO.insertString(xSpreadsheet,
												trans.tslt(templateLanguage,
														"Self"), row,
												column + 1);
						...
						OO.setFontSize(xSpreadsheet, 0, 11, statementPos, statementPos, 16);
						OO.setFontBold(xSpreadsheet, 0, 11, statementPos, statementPos);
						OO.setFontType(xSpreadsheet, 0, 11, statementPos, statementPos,
								"Times New Roman");
			...
			}else if(surveyLevel == 1){// end if surveyLevel Competency
				//Start KB level
					...
					{					
						...
						for(int j=0; j<RT.size(); j++) {
							...
							if(RTCode.equals("CP")) {
								...
								totalAdd = totalRater(RTID, compID,-1, "ADD%", surveyLevel);


								totalDirect = totalRater(RTID, compID, -1,"DIR%", surveyLevel);
								totalIndirect = totalRater(RTID, compID, -1,"IDR%", surveyLevel);
								totalBOSS = totalRater(RTID, compID, -1,"BOSS%", surveyLevel);

								// Re-locate and modified codes to include Peers
								// and Subordinates by Desmond 21 Oct 09
								totalAll = totalSup + totalPeer + totalDirect
										+ totalIndirect + totalSub + totalAdd + totalSub;
								...
								for(int k=0; k<result.size(); k++) {
									...
									case 1 : 
										...
										break;
									case 2 : 
										t = "Superior"; // Change from Supervisors to Superior, Desmond 22 Oct 09
										arrN[iN] = totalSup;
										iN++;
										// Change by Santoso 22/10/08
										// the total rater will be printed later below
								...
								// and Subordinates by Desmond 21 Oct 09
								totalAll = totalSup + totalPeer + totalDirect
										+ totalIndirect + totalSub + totalOth + totalAdd + totalBOSS ;
						...
						for(int j=0; j<KBList.size(); j++) {
							...
							for(int k=0; k<RT.size(); k++) {
								...
								if(RTCode.equals("CP")) {
									...
									totalAdd = totalRater(RTID, compID,KBID, "ADD%", surveyLevel);
									totalBOSS = totalRater(RTID, compID,KBID, "BOSS%", surveyLevel);
	...
	public void InsertCompetency(int reportType, int SurveyID, int breakCPR, 
			Vector combinedRateGroup, String newGroupName) throws SQLException, Exception {
		...
		if (breakCPR == 0) { // not breaking down the CPR(All)
			...
			//PEER, SUB,SUP...
			Vector<String> allRaterType = totalRaterType(surveyID, combinedRateGroup, newGroupName,"CP");
			allRaterType = rearrangeCat(allRaterType);
			int totalType = allRaterType.size();
			// CPR FPR
			Vector<String> allOtherType = otherRT(surveyID);

			String[] Rating = new String[totalType + totalOtherRT];
			
			//fill in with additional code like CPR
			int curPtr = 0;
			for(int n = 0; n < totalOtherRT; n++){
				Rating[n] = allOtherType.get(n);
				curPtr = n+1;
			}
			//fill in with nomral rater code
			for(int n = 0; n < totalType; n++){
				Rating[n + curPtr] = allRaterType.get(n);
			}
			int s = 5;
			...
			OO.insertString(xSpreadsheet,
					trans.tslt(templateLanguage, "COMPETENCY REPORT"), row, 0);
			OO.mergeCells(xSpreadsheet, 0, 7, row,
					row);
			OO.setFontSize(xSpreadsheet, 0, 11, row, row, 16);
			OO.setFontType(xSpreadsheet, 0, 11, row, row,
					"Times New Roman");
				...
				for (int i = 0; i < vComp.size(); i++) {
					...
					for (int j = 0; j < RT.size(); j++) {
						...
						if (RTCode.equals("CP")) {
							...
							totalAll = totalSup + totalPeer + totalDirect
									+ totalIndirect + totalSub + totalOth + totalAdd + totalBOSS ;
							System.out.println(" Line 8005 - Peer : " + totalPeer + "Sub : " + totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS);
							// check if combination happened
							boolean combineOccur = false;
							int numofAffectedRater = 0;
							Vector<Integer> affectedGroup = new Vector<Integer>();
							for (int k = 0; k < result.size(); k++) {
								...
								case 1:
									...
									break;
								case 2:
									t = "Superior"; // Change from Supervisors
													// to Superior, Desmond 22
													// Oct 09
									arrN[iN] = totalSup;
									iN++;
									// Change by Santoso 22/10/08
								...
								// Added case 5 and 6 to cater to splitting of
								// Peers and Subordinates by Desmond 21 Oct 09
								case 5:
									t = "Subordinates";
									arrN[iN] = totalSub;
									iN++;
								...
								case 11:
									...
									iN++;
									try{
										totalRater[getArrIndex(t,Rating,0)] = totalBOSS;
										Result[getArrIndex(t, Rating,0)] = Double
											.parseDouble(arrOther[2]);
									}catch(Exception e){
										combineOccur = true;
										numofAffectedRater += totalBOSS;
										affectedGroup.add(type);
									}
									break;
								}
						...
						} else if (RTCode.equals("CPR") || RTCode.equals("FPR")) {// comp level
							...
							if (result.size() > 0) {
								...
								totalAdd = totalRater(2, compID, -1, "ADD%",
										surveyLevel);
								totalBOSS = totalRater(2, compID, -1, "BOSS%",
										surveyLevel);

								Result[getArrIndex("All",Rating,1)] = Double
										.parseDouble(arrOther[2]);
								if (RTCode.equals("CPR")) {
								...
								totalRater[getArrIndex("All",Rating,1)] = totalAll;
								// 11/12/2009 Denise
								// Change position to display CPR/FPR on the top
					...
					// totalRater.length);
					int[] rowPos = prepareCells(xSpreadsheet, row, totRaterType);
					System.out.println("Line 8322  : " + rowPos.length);
					row++; // start draw chart from here
					OO.setFontSize(12);
				...
				for (int i = 0; i < vComp.size(); i++) {
					...
					for (int j = 0; j < RT.size(); j++) {
						...
						if (RTCode.equals("CP")) {
							...
							totalAll = totalSup + totalPeer + totalDirect
									+ totalIndirect + totalSub + totalOth + totalAdd + totalBOSS ;
							System.out.println(" Line 8669 - " + statement + " Peer : " + totalPeer + " SUP :  " +  totalSup + " Sub : "
									+ totalSub + " totalAdd : " + totalAdd + " Total Boss : " + totalBOSS + " Total self : " + totalSelf + " Total all : " + totalAll
									+ " Total direct : " + totalDirect+ " Total indriect : " + totalIndirect);
							...
							for (int k = 0; k < result.size(); k++) {
								...
								case 4:
									...
									break;
								// Commented away to remove CP(Others) to cater
							...
							if(combineOccur){
								totalRater[getArrIndex(newGroupName,Rating,0)] = numofAffectedRater;
								Result[getArrIndex(newGroupName,Rating,0)] = C.CombineGroupMeanResult(surveyID,targetID,affectedGroup, compID, KBID, false);
								//System.err.println("")
							}
						} else if (RTCode.equals("CPR") || RTCode.equals("FPR")) {
							// Change by Santoso 22/10/08
							...
							totalSub = totalRater(RTID, compID, -1, "SUB%",
									surveyLevel);
							totalBOSS = totalRater(RTID, compID, -1,
									"BOSS%", surveyLevel);
							...
							totalAll = totalSup + totalPeer + totalDirect
									+ totalIndirect + totalSub + totalOth + totalAdd + totalBOSS ;
							System.out.println(" Line 8923 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
									+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
							arrN[iN] = totalAll;
							iN++;
							...
							if (iReportType == 1) {
								System.out.println("insertCompetency() - iReportType = 1 so replace all \n"); // To
																											// Remove,
								Rating[getArrIndex("All",Rating,1)]	= 	Rating[getArrIndex("All",Rating,1)].replaceAll("\n", "");
							}

							totalRater[getArrIndex("All",Rating,1)] = totalAll;
							Result[getArrIndex("All",Rating,1)] = CompTrimmedMeanforAll(RTID,compID);
					...
					if (splitOthers == 0) {
						...
						//totRaterType -= 1; need to check the conditions for which this statement was written
					}
					...
					for (int j = 0; j < KBList.size(); j++) {
						...
						for (int k = 0; k < RT.size(); k++) {
							...
							if (RTCode.equals("CP")) {
								...
								totalAdd = totalRater(RTID, compID, KBID,
										"ADD%", surveyLevel);
								totalBOSS = totalRater(RTID, compID, KBID,
										"BOSS%", surveyLevel);
								...
								totalAll = totalSup + totalPeer + totalDirect
										+ totalIndirect + totalSub + totalOth + totalAdd + totalBOSS ;
								System.out.println(" Line 9187 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
										+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
								// check if combination happened
								boolean combineOccur = false;
								int numofAffectedRater = 0;
								...
								for (int l = 0; l < result.size(); l++) {
									...
									case 7:
										...
										try{
											totalRater[getArrIndex(t,Rating,0)] = totalAdd;
											Result[getArrIndex(t, Rating,0)] = Double.parseDouble(arr[2]);
										}catch(Exception e){
											combineOccur = true;
											numofAffectedRater += totalAdd;
											affectedGroup.add(type);
								...
								if (result.size() != 0) { // && k<result.size())
									...
									totalAll = totalSup + totalPeer + totalDirect
											+ totalIndirect + totalSub + totalOth + totalAdd + totalBOSS ;
									System.out.println(" Line 9470 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
											+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
									...
									// customization (16 Dec 2009 Qiao Li)
									totalRater[getArrIndex("All",Rating,1)] = totalAll;
									Result[getArrIndex("All", Rating, 1)] = Double.parseDouble(arr[2]);
									//totalRater[totalType] = totalAll;
									...
									// change from 5 to totalType for
									// customization (16 Dec 2009 Qiao Li)
									double CPRFPR_Result = Result[getArrIndex("All",Rating,1)];
									int CPRFPR_TotalRating = totalRater[getArrIndex("All",Rating,1)];
									/*
									for (int x = totalType; x > 0; x--) {
										totalRater[x] = totalRater[x - 1];
										Result[x] = Result[x - 1];
						...
						// /Denise 07/01/2009
						if (count == 0) { // move table two lines down
							//OO.insertRows(xSpreadsheet, 0, 10, row + 1,
							//		row + 3, 2, 1);
							row += 2;
						}
					}// while KBList
			...
			int totalOtherRT = totalOtherRT(); // total other rating task beside
												// CP
			int total = totalGroup() + totalOtherRT + 1; // 1 for all
			Vector<String> allRaterTypeCPR = totalRaterType(surveyID, combinedRateGroup, newGroupName, "CPR");
			Vector<String> allRaterTypeCP = totalRaterType(surveyID, combinedRateGroup, newGroupName, "CP");
			int totalType = allRaterTypeCPR.size();
			...
			//fill in with nomral rater code
			for(int n = 0; n < totalType*2; n+=2){
				Rating[n] = allRaterTypeCPR.get(n/2);
				Rating[n+1] = allRaterTypeCP.get(n/2);
			}
			
			for(int n = 0; n < Rating.length; n++){
				System.err.println("first "+Rating[n]);
			}
					...
					for (int j = 0; j < RT.size(); j++) {
						...
						if (RTCode.equals("CP")) {
							...
							totalAll = totalSup + totalPeer + totalSub
									...
									+ totalOth + totalAdd +totalBOSS;
							System.out.println(" Line 9825 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
									+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
							...
							System.out.println(" Line 9976 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
									+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
							// check if combination happened
							boolean combineOccur = false;
							int numofAffectedRater = 0;
								...
								case 2:
									...
									if (iReportType == 1)
										Rating[2] = Rating[2].replaceAll("\n",
												" ");
									Result[2] = Double.parseDouble(arrOther[2]);*/
								...
								case 4:
									...
									if (iReportType == 1)
										Rating[6] = Rating[6].replaceAll("\n",
												" ");
									Result[6] = Double.parseDouble(arrOther[2]);*/
								...
								case 5:
									...
									if (iReportType == 1)
										Rating[8] = Rating[8].replaceAll("\n",
												" ");
									Result[8] = Double.parseDouble(arrOther[2]);*/
					...
					for (int j = 0; j < RT.size(); j++) {
						...
						if (RTCode.equals("CP")) {
							...
							totalAll = totalSup + totalPeer + totalSub
									...
									+ totalOth + totalAdd +totalBOSS;
							System.out.println(" Line 10384 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
									+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
							...
							totalAll = totalSup + totalPeer + totalSub
									...
									+ totalOth + totalAdd +totalBOSS;
							System.out.println(" Line 10524 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
									+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
							...
							if(combineOccur){
								totalRater[getArrIndex(newGroupName,Rating,1)] = numofAffectedRater;
								Result[getArrIndex(newGroupName,Rating,1)] = C.CombineGroupMeanResult(surveyID,targetID,affectedGroup, compID, KBID, true);
								//System.err.println("")
							}
						...
						for (int k = 0; k < RT.size(); k++) {
							...
							if (RTCode.equals("CP")) {
								...
								totalAll = totalSup + totalPeer + totalSub
										...
										+ totalOth + totalAdd +totalBOSS;
								System.out.println(" Line 10817 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
										+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
								...
								totalAll = totalSup + totalPeer + totalSub
										...
										+ totalOth + totalAdd +totalBOSS;
								System.out.println(" Line 10949 - Peer : " + totalPeer + "SUP :  " +  totalSup + "Sub : "
										+ totalSub + " totalAdd : " + totalAdd + "Total Boss : " + totalBOSS + "Total self : " + totalSelf);
	...
	public void drawChart(String Rating [], double Result [], int[] totalRater, int[] rowPos, int maxScale, int splitOthers) throws IOException, Exception 
	{
		...
		if (splitOthers == 0) {
			...
			int iOth = 0, iSub = 0, iPeer = 0, iAdd = 0, iDir = 0, iIndir = 0, iBOSS =0;
			for (int i = 0; i < Rating.length; i++) {
		...
		if (iReportType == 2) {
			...
			// draw chart
			XTableChart xtablechart = OO.getChart(xSpreadsheet, xSpreadsheet2,
					c, c + 1, row - 1, r - 1, Integer.toString(row), 10000,
					7800, row, 2);
			OO.setFontSize(8);
	 ...
	 * if splitOthers ==0, show "Others", if splitOthers==1, show "Subordinates" and "Peers"
	 * */
	public void drawChartBreakCPR(String Rating [], double Result [], int[] totalRater, int[] rowPos, int maxScale, int splitOthers, int totalBar) throws IOException, Exception 
	{
		...
		if(splitOthers == 0){
			...
			//for CP
			formatter.setRoundingMode(RoundingMode.HALF_UP);
			String value = formatter.format(((totalRater[iSub]*Result[iSub]) + (totalRater[iPeer]*Result[iPeer]) + (totalRater[iOth]*Result[iOth]) + (totalRater[iAdd]*Result[iAdd]))/(totalRater[iSub] + totalRater[iPeer] + totalRater[iOth] + totalRater[iAdd]));
			//Store the value into Others row in Result
			Result[iOth] = Double.parseDouble(value);
			//Store the total number of people into Others row in totalRater
			totalRater[iOth] += totalRater[iSub] + totalRater[iPeer] + totalRater[iAdd];

			//for CPR
			value = formatter.format(((totalRater[iSub-1]*Result[iSub-1]) + (totalRater[iPeer-1]*Result[iPeer-1]) + (totalRater[iAdd-1]*Result[iAdd-1]) + (totalRater[iOth-1]*Result[iOth-1]))/(totalRater[iSub-1] + totalRater[iPeer-1] + totalRater[iAdd-1] + totalRater[iOth-1]));
			//Store the value into Others row in Result
			Result[iOth-1] = Double.parseDouble(value);
			//Store the total number of people into Others row in totalRater
			totalRater[iOth-1] += totalRater[iSub-1] + totalRater[iPeer-1] + totalRater[iAdd-1];
		}
		...
		for(int i=0; i<Rating.length; i++) {
			...
			if (Rating[i] != null ) {
				//identify "Self" (Qiao Li 17 Dec 2009)
				if (Rating[i].equals("CPR(Self)")||Rating[i].equals("CP(Self)")){
					selfIdx = i;
					continue;
				}
				 ...
				 * Date: 17 Dec 2009
				 */
				if ((splitOthers== 1 && !Rating[i].equals("CP(Others)") && !Rating[i].equals("CPR(Others)"))||
						(splitOthers == 0 && !Rating[i].equals("CP(Subordinates)")&& !Rating[i].equals("CP(Peers)")&& !Rating[i].equals("CPR(Subordinates)")&& !Rating[i].equals("CPR(Peers)")&& !Rating[i].equals("CP(Additional)")&& !Rating[i].equals("CPR(Additional)"))){
					r++;
		...
		if (selfIdx != -1) {
			...
			r++;
			OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, Rating[selfIdx-1]), r, c + 2);
			OO.mergeCells(xSpreadsheet, c + 2, c + 3, r, r);
		...
		if(iReportType == 2) 
		{
			...
			for(int i=0; i<Rating.length; i++) {
				...
				{	
					//identify "self" (Qiao Li 17 Dec 2009)
					if (Rating[i].equals("CP(Self)")||Rating[i].equals("CPR(Self)")){
						selfIdx = i;
						continue;
					}
					/* Change (s): added in flags
					...
					if ((splitOthers== 1 && !Rating[i].equals("CP(Others)") && !Rating[i].equals("CPR(Others)"))||
						...
						OO.insertNumeric(xSpreadsheet2, Result[i], r, c+1);
						OO.insertNumeric(xSpreadsheet, totalRater[i], rowPos[cellIdx], 0);
					//	System.out.println("2: " +  Rating[i] + " " +  totalRater[i] + " , " +  rowPos[cellIdx]);
						r++;
						cellIdx++;
					}
				}			
			...
			if (selfIdx != -1) {
				OO.insertString(xSpreadsheet2, trans.tslt(templateLanguage, Rating[selfIdx-1]), r, c);
				OO.insertNumeric(xSpreadsheet2, Result[selfIdx-1], r, c + 1);
				OO.insertNumeric(xSpreadsheet, totalRater[selfIdx-1], rowPos[cellIdx], 0);
				r++;
				cellIdx++;
				OO.insertString(xSpreadsheet2, trans.tslt(templateLanguage, Rating[selfIdx]), r, c);
				OO.insertNumeric(xSpreadsheet2, Result[selfIdx], r, c + 1);
			...
			XTableChart xtablechart = OO.getChart(xSpreadsheet, xSpreadsheet2, c, c+1, row-1, r-1, Integer.toString(row), 10000, 7700, row, 2);
	...
	public void InsertClusterComments() throws SQLException, IOException, Exception 
	{
		...
		OO.insertString(xSpreadsheet, "NARRATIVE COMMENTS",
				row,0);
		OO.setFontSize(xSpreadsheet, 0, 11, row, row, 16);
		OO.setFontBold(xSpreadsheet, 0, 11, row, row);
		OO.setFontType(xSpreadsheet, 0, 11, row, row,
				"Times New Roman");
			...
			for(int l=0; l<vClust.size();l++){
				...
				for(int i=0; i<vComp.size(); i++) {
					...
					if(blnOthIncluded){//added by Ping Yang on 11/08/08, check raters assigned
					   ...
					   Updated by: Qiao Li 23 dec 2009*/
						if (splitOthers==1){
							...
							// Added codes to insert Additional Raters' comments ~ Albert 5 Sept 2012
							boolean blnAddCommentExist = false;	//Added to get rid of extra '-'s
							for(int k=0; k<addComments.size(); k++) {
								...
								if (start == 0) {
									//Allow dynamic translation, Chun Yeong 1 Aug 2011
									OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, "Additional Rater(s)"),
											row, column + 1);
									OO.setFontBold(xSpreadsheet, startColumn,
											endColumn, row, row);
									OO.setFontItalic(xSpreadsheet, startColumn,
					...
					if(currentPageHeight >pageHeightLimit){//adding the table will exceed a single page, insert page break
						OO.insertPageBreak(xSpreadsheet, startColumn, endColumn, startBorder); 
						   ...
						   Updated by: Qiao Li 23 dec 2009*/
							if (splitOthers==1){
								...
								for (int k = 0; k < addComments.size(); k++) {
									...
									if (start == 0) {
										//Allow dynamic translation, Chun Yeong 1 Aug 2011
										OO.insertString(xSpreadsheet,trans.tslt(templateLanguage, "Additional Rater(s)"), row, column + 1);
										OO.setFontBold(xSpreadsheet, startColumn,endColumn, row, row);
										OO.setFontItalic(xSpreadsheet, startColumn,endColumn, row, row);
										start++;
										row++;
									...
									if (!comment.trim().equals("")) {OO.insertString(xSpreadsheet,"- "+ UnicodeHelper.getUnicodeStringAmp(comment),row, column + 1);
									OO.mergeCells(xSpreadsheet, column + 1,endColumn, row, row);
									OO.setRowHeight(xSpreadsheet, row,column + 1, ROWHEIGHT* OO.countTotalRow(comment,85));
									OO.setCellAllignment(xSpreadsheet,startColumn, endColumn, row, row,2, 1);
	...
	public void InsertComments() throws SQLException, IOException, Exception 
	{
		...
		//added by Ping Yang on 11/08/08, check raters assigned
		boolean blnSupIncluded = Q.categoryCommentIncluded(surveyID, targetID, "SUP%");
		boolean blBOSSIncluded = Q.categoryCommentIncluded(surveyID, targetID, "BOSS%");
		boolean blnOthIncluded = Q.OthCommentIncluded(surveyID, targetID);
		...
		if(surveyLevel == 0) {
			...
			for(int i=0; i<vComp.size(); i++) {
				...
				Vector peerComments = getComments("PEER%", compID, KBID);
				Vector subComments = getComments("SUB%", compID, KBID);
				Vector addComments = getComments("ADD%", compID, KBID); //get comments for additional raters

				Vector BOSSComments = getComments("BOSS%", compID, KBID);
				...
				if(blBOSSIncluded){//added by Ping Yang on 11/08/08, check raters assigned
					boolean blnBOSSCommentExists  = false;//Added by ping yang on 31/7/08 to get rid of extra '-'s
					
					for(int j=0; j<BOSSComments.size(); j++) {
						String [] arr = (String[])BOSSComments.elementAt(j);
						if(start == 0) {


							OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, "Boss"), row, column+1); 
							OO.setFontBold(xSpreadsheet, startColumn, endColumn, row, row);
							OO.setFontItalic(xSpreadsheet, startColumn, endColumn, row, row);
				...
				if(blnOthIncluded){//added by Ping Yang on 11/08/08, check raters assigned
					...
					if (splitOthers==1){
						...
						for(int k=0; k<addComments.size(); k++) {
							...
							if (start == 0) {
								//Allow dynamic translation, Chun Yeong 1 Aug 2011
								OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, "Additional Rater(s)"), row,
										column + 1);
								OO.setFontBold(xSpreadsheet, startColumn,
										endColumn, row, row);
								OO.setFontItalic(xSpreadsheet, startColumn,
			...
			for(int i=0; i<vComp.size(); i++) {
				...
				for(int j=0; j<KBList.size(); j++) {
					...
					if(blBOSSIncluded){//added by Ping Yang on 11/08/08, check raters assignedblBOSSIncluded
						boolean blnBOSSCommentExist = false;//Added by ping yang on 31/7/08 to get rid of extra '-'s

						for(int k=0; k<BossComments.size(); k++) {
							String [] arr = (String[])BossComments.elementAt(k);

							String comment = arr[1];	

							if(start == 0) {
								OO.insertString(xSpreadsheet, trans.tslt(templateLanguage, "BOSS "), row, column+1); // Change from Supervisors to Superior, Desmond 22 Oct 09
								OO.setFontBold(xSpreadsheet, startColumn, endColumn, row, row);
								OO.setFontItalic(xSpreadsheet, startColumn, endColumn, row, row);
	...
	public Vector Report(int surveyID,int divisionID,int departmentID,int groupID,int targetID, int pkUser, String fileName,
			int type, String chkNormative, String chkGroupCPLine,
			String chkSplit, String chkBreakCPR, int lang, String template,
			boolean weightedAvg,int round,Vector raterCodeList,String newGroupName,boolean surveyOverview) 
	...
	// End add by Tracy 01 Sep 08
	{
		try {
			...
			newRaterCodeList=new Vector();
			oriRaterCodeList = new Vector();
			if(raterCodeList!=null &&raterCodeList.size()>0){
				
				Vector originalRaterCodes = getSplitGroups(surveyID);
				
				for(int i=0;i<originalRaterCodes.size();i++){
					String raterCodes = (String) originalRaterCodes.elementAt(i);
					boolean newGroup=false;
					
					for(int j=0;j<raterCodeList.size();j++){
						String newCodes = (String) raterCodeList.elementAt(j);
						
						if((raterCodes.trim()).equalsIgnoreCase(newCodes.trim())){
							newGroup=true;
					...
					if(newGroup==true){
						...
						newRaterCodeList.add(raterCodes);
					}else{
						
						oriRaterCodeList.add(raterCodes);
					}
				}
			}
	
//			System.out.println("ORI: "+ oriRaterCodeList.size());
//			for(int z=0;z<oriRaterCodeList.size();z++){
//				System.out.println("ORI: "+ (String)oriRaterCodeList.elementAt(z));
//			}
//			System.out.println("new: "+newRaterCodeList.size());
//			for(int z=0;z<newRaterCodeList.size();z++){
//				System.out.println("new: "+ (String)newRaterCodeList.elementAt(z));
//			}
			/*
			...
			InitializeExcel(fileName, template);
			...
			try{
				...
				errors.add("Initialization of survey failed.");
				e.printStackTrace();
			}
			try{
				System.out.println("Inserting replacement");
				Replacement();
			}catch(Exception e){
				errors.add("Replacement of key tags failed.");
				e.printStackTrace();
			}
			try{
				System.out.println("Inserting Rating scale list");
			...
			try{
				if(surveyOverview){
					System.out.println("Inserting survey overview");
					InsertSuveyOverview(surveyID, targetID);
				}
			}catch(Exception e){
				errors.add("Unable to initialize survey overview");
				e.printStackTrace();
			}
			PrelimQuestionController pqc = new PrelimQuestionController();
			try{
				if (pqc.getQuestions(surveyID).size() > 0) {
					System.out.println("Inserting Prelim Questions");
					InsertPrelimQuestions();
				}
			}catch(Exception e){
				errors.add("Unable to insert prelim questions.");
				e.printStackTrace();
			}
			...
			// InsertRatingScaleList();
			try{
				System.out.println("Inserting CP Vs CPR");
				InsertCPvsCPR();
			}catch(Exception e){
				errors.add("CP vs CPR component failed to initialized");
				e.printStackTrace();
			}

			try{
				if (Integer.parseInt(surveyInfo[9]) == 0) { // without cluster
					System.out.println("Inserting gap without clusters");
					...
					OO.deleteRows(xSpreadsheet, startColumn, endColumn,
							tempRow - 4, tempRow + 8, 11, 1);
					InsertGapTitle(surveyID);
					InsertGap();
				} else { // with cluster
					System.out.println("Inserting gap with clusters");
				...
				errors.add("Failed to initialize 'gap' component.");
				e.printStackTrace();
			}
			...
			if (chkNormative != "") {
				System.out.println("Inserting normative");
				InsertNormative();
			...
			try{
				System.out.println("Inserting comp gap");
				InsertCompGap(surveyID);
			}catch(Exception e){
				errors.add("Failed to initialize insertion of competency gap.");
				e.printStackTrace();
			}
			// End add by Tracy 01 Sep 08**********************
			...
			System.out.println("Inserting profile legend");
			//InsertProfileLegend();
			
			int rowNumber = row;
			try{
				if (Integer.parseInt(surveyInfo[9]) == 0) { // without cluster
					System.out.println("Inserting competency");
					try{
					InsertCompetency(type, surveyID, breakCPR, raterCodeList, newGroupName);
					}catch(Exception e){
						e.printStackTrace();
					}
				} else { // with cluster
					System.out.println("Inserting clustercompetency");
					InsertClusterCompetency(type, surveyID, breakCPR, raterCodeList, newGroupName);
					
				}
			}catch(Exception e){
				errors.add("Failed to initialize report component.");
			}
			...
			if (hasImportance(surveyID)) {
				...
				OO.deleteRows(xSpreadsheet, startColumn, endColumn, iRow,
						iRow + 3, 2, 1);
			}
			...
			// Insert the header
			OO.insertString(xSpreadsheet,
					trans.tslt(templateLanguage, "BLIND SPOTS ANALYSIS"), row,
					0);
			OO.setFontBold(xSpreadsheet, startColumn, endColumn, row, row);
			OO.setFontSize(xSpreadsheet, startColumn, endColumn, row, row, 16);
			...
			if (hasRatedSelfKBLevel(surveyID, targetID)
				...
				if (Integer.parseInt(surveyInfo[9]) == 0) { // without cluster
					System.out.println("Inserting blindspot");
				...
				OO.mergeCells(xSpreadsheet, startColumn, endColumn - 1, row,
						row);
				row += 2;
			}
			lastPageRowCount = 0;
			...
			//  Comments are moved to under each competency graph 
			if(included ==  1||selfIncluded==1){
				  if(Integer.parseInt(surveyInfo[9])==0){
			  //without cluster 
					  System.out.println("Inserting Comments..");
				  InsertComments(); 
				  } else{
					  System.out.println("Inserting Cluster Comments..");
			...
			try{
				...
				if (aqc.getQuestions(surveyID).size() > 0){
					System.out.println("Inserting additional qns");
					InsertAdditionalQuestions();
				}
			}catch(Exception e){
				errors.add("Failed to initialize insertion of additional questions");
				e.printStackTrace();
			}

			int rowEnd = row;
		//	testpagelength();
			//OO.insertRows(xSpreadsheet, 0, totalColumn, rowNumber, row, row-rowNumber,rowNumber);
			...
			try{
				System.out.println("Inserting item frequency");
				insertItemFrequency(surveyID);
			}catch(Exception e){
				errors.add("Failed to intialize insertion of item frequency table.");
				e.printStackTrace();
			}
			
			try{
				System.out.println("Inserting targetrank");
				printTargetRank();
			}catch(Exception e){
				errors.add("Failed to initialize insertion of individual ranking in the group");
				e.printStackTrace();
			}
			try{
				System.out.println("Inserting competency rank table");
				insertCompetencyRankTable();
			}catch(Exception e){
				errors.add("Failed to initialize competency rank table");
				e.printStackTrace();
			}
			try{
			OO.insertHeaderFooter(	xDoc,	surveyInfo[1],surveyInfo[6] + "\n" + UserName() + "\n",	"Date of printing: "	+ temp
							+ "\n"+ "Copyright \u00a9 3-Sixty Profiler\u00AE is a product of Pacific Century Consulting Pte Ltd.",1,5);
			}catch(Exception e){
				errors.add("Failed to initialize Header/Footer.");
				e.printStackTrace();
			}
	...
	/*
	 * Added this method to dynamically add information to the Individual Report Information (Interpreting Report)
	 * By Albert 7 Sept 2012
	 */
		...
		if (vCompDisplay.size() != 0) {
			...
			for (Object obj : vCompDisplay) {
				...
				if (currentPageHeight > pageHeightLimit
					...
					// Draw the border
					OO.setTableBorder(xSpreadsheet, startColumn, endColumn - 1,
							row - 1, row - 1, false, false, false, false,
							false, true);
					// Insert page break
					OO.insertPageBreak(xSpreadsheet, startColumn, endColumn,
							row);
				...
				// Insert Competency Name
				// Added translation to competency name, Chun Yeong 1 Aug 2011
				OO.insertString(xSpreadsheet,compCount+ ". "+ UnicodeHelper
										.getUnicodeStringAmp(getTranslatedCompetency(
												temp[1]).elementAt(0)
				...
				// Centralize and draw border for the data
				OO.setCellAllignment(xSpreadsheet, posCol, selfCol, row, row,
						1, 2);
						...
						for (int k = 0; k < RT.size(); k++) {
							...
							if (RTCode.equals("CP")) {
								...
								if ((KBResult[0] > KBResult[1])
									...
									OO.insertString(xSpreadsheet,"KB: "	+ getTranslatedKeyBehavior(UnicodeHelper								
															.getUnicodeStringAmp(KBName)),row, firstColumn);
									OO.setRowHeight(xSpreadsheet, row, 1,
											ROWHEIGHT * OO.countTotalRow(KBName, 135));
									OO.mergeCells(xSpreadsheet, 0, posCol - 1,	row, row+1);
									OO.mergeCells(xSpreadsheet, 0, posCol - 1,	row, row);
	...
	public void InsertRatingScaleList() throws Exception{
		...
		try{ 
			...
			for(int i=0; i<1; i++) {
						...
						if (low == high){
							...
							while(low <= high) {						
								if(firstTime) {
									...
									if(low == high) temp += Integer.toString(low);
									low++;						
								}
							}
						}
				...
				String paragraph = "All the graphs in this report with a "+ratingScale+" point scale use the following rating scale description: ";
				...
				if(iNoCPR==1)//if no CPR (CP Only)
					OO.findAndReplace(xSpreadsheet, "<CR+1>",Integer.toString(CRPlusOne));
			}
	...
	public boolean hasImportance(int surveyID){
		...
		try{
			...
			System.out.println("IndividualReport.java - hasImportance - "+ex.getMessage());
		}
		finally{
			ConnectionBean.closeRset(rs); //Close ResultSet
			ConnectionBean.closeStmt(st); //Close statement
	...
	/*
	 * To check if a particular survey and target rate himself or not
	 * for Competency level survey
	 * @author Albert
	 */
	public boolean hasRatedSelfCompLevel(int surveyID, int targetID){
		...
		query += "WHERE tblAssignment.SurveyID = " +surveyID+" AND tblAssignment.TargetLoginID = "+targetID+" AND tblAssignment.RaterCode = 'SELF'";
		...
		try{
			...
			System.out.println("IndividualReport.java - hasRatedSelfCompLevel - "+ex.getMessage());
		}
		finally{
			ConnectionBean.closeRset(rs); //Close ResultSet
			ConnectionBean.closeStmt(st); //Close statement
	...
	public boolean hasRatedSelfKBLevel(int surveyID, int targetID){
		...
		try{
			...
			System.out.println("IndividualReport.java - hasRatedSelfKBLevel - "+ex.getMessage());
		}
		finally{
			ConnectionBean.closeRset(rs); //Close ResultSet
			ConnectionBean.closeStmt(st); //Close statement
	...
	public String getName(int targetId){
		...
		query += "select givenName,familyname from [user] where pkuser="+targetId;
		...
		try{
			...
			if(rs.next()) {
				name = rs.getString("givenName") + " " + rs.getString("familyName");
			}
		}catch(Exception ex){
			System.out.println("IndividualReport.java - getName - "+ex.getMessage());
		}
		finally{
			ConnectionBean.closeRset(rs); //Close ResultSet
			ConnectionBean.closeStmt(st); //Close statement
	...
	public void InsertPrelimQuestions() {
		try {
			...
			for (int i = 0; i < questions.size(); i++) {
				...
				OO.setCellAllignment(xSpreadsheet, startColumn, column + 9,
						row, row, 2, 1); // Set alignment of competency name to
				...
				if (ratingScaleId != -1) { // using rating scale
					...
					Vector scales = pqController.getAllRatingInVector(ratingScaleId);
					...
					for (int a = 0; a < scales.size(); a++) {
						...
						OO.mergeCells(xSpreadsheet, column + 1, column + 3,
								row, row);
						...
						// OO.mergeCells(xSpreadsheet, column + 10, column + 11,
						// row, row);
						OO.setTableBorder(xSpreadsheet, column + 10,
								column + 11, row, row, false, false, true,
								true, true, true);
						row++;
					}
					row++;
	...
	public void InsertSuveyOverview(int surveyID, int targetID) {
		try {
			...
			OO.insertString(xSpreadsheet,
					trans.tslt(templateLanguage, "Questionnaire"), row,
					column + 3);

			OO.mergeCells(xSpreadsheet, column + 7, column + 10, row, row);

			OO.insertString(xSpreadsheet,
					trans.tslt(templateLanguage, "Familiarity"), row,
					column + 7);
			...
			OO.insertString(xSpreadsheet,
					trans.tslt(templateLanguage, "Distributed"), row,
					column + 3);
			...
			OO.setCellAllignment(xSpreadsheet, 0, 11, startRow - 1, endRow, 1,
					2);
			OO.setFontSize(xSpreadsheet, 0, 11, startRow - 1, endRow, 12);
			OO.setTableBorder(xSpreadsheet, 0, 10, startRow, endRow, true,
	...
	public void insertItemFrequency(int surveyID) throws SQLException,Exception {
		rowFreq = 1;
		columnFreq = 0;
		OO.insertString(xSpreadsheet3,
				trans.tslt(templateLanguage, "ITEM FREQUENCY REPORT"), rowFreq,
				0);
...
for (int i = 0; i < vClust.size(); i++) {
	...
	OO.insertString(xSpreadsheet3,
			...
			rowFreq, 0);
	OO.setFontBold(xSpreadsheet3, startColumnFreq, endColumnFreq,
			rowFreq, rowFreq);
	rowFreq++;
	...
	OO.insertString(xSpreadsheet3,
			...
			columnFreq);
	OO.insertString(xSpreadsheet3,
			trans.tslt(templateLanguage, "Peers"), rowFreq,
			columnFreq + 6);
	OO.insertString(xSpreadsheet3,
	...
	OO.setCellAllignment(xSpreadsheet3, columnFreq, columnFreq + 5,
			rowFreq, rowFreq, 1, 2);
	OO.mergeCells(xSpreadsheet3, columnFreq + 6, columnFreq + 11,
			rowFreq, rowFreq);
	OO.setCellAllignment(xSpreadsheet3, columnFreq + 6,
			columnFreq + 11, rowFreq, rowFreq, 1, 2);
	OO.mergeCells(xSpreadsheet3, columnFreq + 12, columnFreq + 17,
			rowFreq, rowFreq);
	OO.setCellAllignment(xSpreadsheet3, columnFreq + 12,
			columnFreq + 17, rowFreq, rowFreq, 1, 2);
	...
	OO.setTableBorder(xSpreadsheet3, startColumnFreq, endColumnFreq,
			startBorder, rowFreq - 1, true, true, true, true, true,
			true);
	...
	public Vector getTotalNumberOfGroups(int surveyID) throws SQLException {
		...
		String query = "select distinct type as result from tblAvgMean inner join tblassignment on tblassignment.targetloginid =tblavgmean.targetloginid" +
				" where tblassignment.SurveyID=" + surveyID + " and Type<>1 and Type<>10 and tblassignment.round =  " + round;
		...
		try {
			...
			System.out.println("individual.java - getTotalNumberOfGroups - "
					+ ex.getMessage());
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
			ConnectionBean.closeStmt(st); // Close statement
	...
	public void printTargetRank() throws SQLException, IOException, Exception {
		...
		String info = "This report lists the LG PLUS Feedback Recipients in ranked order based on the aggregate of ";
		info += "all their CP (All) scores. The report is in descending order starting with the strongest LG PLUS Feedback Recipients.";
		int row = 0;
		...
		vCPValues = G.sortingWithID(vCPValues, 1); // sorting the CP (All)
													// values
		for(int a=0;a<vCPValues.size();a++){
			String[] abc=(String[]) vCPValues.elementAt(a);
			String tbc = abc[1];
		...
		OO.setCellAllignment(xSpreadsheet5, column + 9, column + 9, row, row,
				1, 2);
		...
		for (int j = 0; i < vCPValues.size(); j++) {
			...
			OO.setFontRemoveItalic(xSpreadsheet5, startColumn, endColumn, row,
					row);
			...
			if (target[i] != null) {
				...
				if (targetID1 == targetID) {
					...
					// row, row);
					OO.setFontBold(xSpreadsheet5, startColumn, endColumn, row,
							row);
					OO.setBGColor(xSpreadsheet5, startColumn, endColumn - 1,
							row, row, 16737792);
		...
		int endBorder = row - 1;
		OO.setTableBorder(xSpreadsheet5, 0, 11, startBorder, endBorder, false,
				false, true, true, true, true);
		...
		try {
			...
			for (int i = 0; i < vClust.size(); i++) {
				...
				if (clusterName.equalsIgnoreCase("SELF")) {
					...
					startRow += 2;
					OO.mergeCells(xSpreadsheet5, column, column + 11, startRow,
							startRow + 1);
					...
					OO.insertString(xSpreadsheet5, clusterName, startRow,
							column);
					OO.setCellAllignment(xSpreadsheet5, 0, 1, startRow, startRow, 1, 2);
					...
					OO.mergeCells(xSpreadsheet5, column + 2, column + 9,
							startRow, startRow);
					OO.insertString(xSpreadsheet5, "Name", startRow, column + 2);
					OO.mergeCells(xSpreadsheet5, column + 10, column + 11,
							startRow, startRow);
					OO.insertString(xSpreadsheet5, "CP Score", startRow,
							column + 10);
					
					OO.setBGColor(xSpreadsheet5, 0, 11, startRow,startRow, BGCOLOR);
					OO.setTableBorder(xSpreadsheet5, 0, 11, startRow, startRow,true,
							true, true, true, true, true);
					OO.setCellAllignment(xSpreadsheet5, 10, 11, startRow, startRow, 1, 2);
				...
				//	rankingTable = sortClusterCompetency(rankingTable);
					...
					for (int k = 0; k < rankingTable.size(); k++) {
						 ...
						 */
						if (target != targetID) {
							OO.mergeCells(xSpreadsheet5, 2, 4, startRow,	startRow);
							OO.insertString(xSpreadsheet5,
									"Feedback Recipient " + (k + 1), startRow,2);
							OO.mergeCells(xSpreadsheet5, column + 10,
									column + 11, startRow, startRow);
							OO.insertNumeric(xSpreadsheet5,
									Math.round(Double.parseDouble(individualScorePair[1])*100.00)/100.00, startRow, 10);
							OO.setCellAllignment(xSpreadsheet5, 10, 11, startRow, startRow, 1, 2);
							startRow++;
						} else {
							OO.mergeCells(xSpreadsheet5, 2, 4, startRow,	startRow);
					...
					}OO.setTableBorder(xSpreadsheet5, 0, 11, startOfTable, startRow-1, false,
							false, true, true, true, true);
					...
					for (int j = 0; j < vComp.size(); j++) {
						...
						OO.insertString(xSpreadsheet4, competencyName, listing,
								competencyStartCol);
						OO.setTableBorder(xSpreadsheet4, competencyStartCol, competencyStartCol+1, listing, listing, true,
								true, true, true, true, true);
						...
						OO.setBGColor(xSpreadsheet4, competencyStartCol, competencyStartCol+1, listing,listing, BGCOLOR);
						listing++;
						/*
						...
						for (int k = 0; k < rankingTable.size(); k++) {
							...
							if (target != targetID) {
								...
								OO.setBGColor(xSpreadsheet4,
										...
										listing, 16737792);
								OO.setFontBold(xSpreadsheet4, competencyStartCol, competencyStartCol+1, listing, listing);
								listing++;
							}
					...
					OO.setBGColor(xSpreadsheet4, 0, 1+1, startRow+1,startRow+1, BGCOLOR);
					...
					for (int p = 0; p < clusterScores.size(); p++) {
						...
						if (target != targetID) {
							...
							OO.insertNumeric(xSpreadsheet4,Math.round((Double.parseDouble(clusterScoreTable[1])/ vComp.size())*100.00)/100.00,insertClusterScores,2);
							insertClusterScores++;
						} else {
							OO.insertString(xSpreadsheet4,
									clusterScoreTable[0], insertClusterScores,
									1);

							OO.insertNumeric(xSpreadsheet4,Math.round((Double.parseDouble(clusterScoreTable[1])/ vComp.size())*100.00)/100.00,insertClusterScores,2);
							OO.setFontBold(xSpreadsheet4, 0, 2, insertClusterScores, insertClusterScores);
				 ...
				 * cluster.
				 */
				vComp = CompetencyByName();
				System.out.println("Size of comp" + vComp.size());
				...
				for (int j = 0; j < vComp.size(); j++) {
					...
					Vector rankingTable = getCompetencyRankingData(compID);
					...
					System.out.println("COMPETENCY RANKING TABLE: " + competencyName + "Number of people : " + numberOfPeople);
						for (int k = 0; k < rankingTable.size(); k++) {
							String[] individualScorePair = (String[]) rankingTable
									.elementAt(k);
	
							int target = Integer
									.parseInt(individualScorePair[0]);
							/*
							 * Blank out Name is target is not the user of this
							 * individual Report
							 */
							if (target != targetID) {
								OO.insertString(xSpreadsheet4,"Feedback Recipient " + (k + 1),listing, competencyStartCol);
								OO.insertNumeric(xSpreadsheet4,	Double.parseDouble(individualScorePair[1]), listing,competencyStartCol + 1);
								listing++;
							} else {
								OO.insertString(xSpreadsheet4,
										individualScorePair[2], listing,
										competencyStartCol);
			...
			System.out.println("insertCopetencyRankTable - Individualreport.java"+ e);
			e.printStackTrace();
		}
	}
	...
	public int getSelfScore(int compID) throws SQLException {
		...
		query = query + "AND tblAssignment.RaterCode LIKE '%SELF%' and tblassignment.round = " + round;;
	...
	public int[] getSurveyRaterStatus(int targetLoginID, String raterCode) {
		{
			...
			query += " inner join tbl_PrelimQn c on a.FKPrelimQnID=c.PrelimQnID where TargetLoginID="
					+ targetLoginID + " and RaterCode like '" + raterCode + "' and b.round =" + round;
			if (divisionID != 0)
				query = query + " AND b.FKTargetDivision = "
						+ divisionID;

			if (departmentID != 0)
				query = query + " AND b.FKTargetDepartment = "
						+ departmentID;

			if (groupID != 0)
				query = query + " AND b.FKTargetGroup = "
						+ groupID;
			...
			try {
				...
				System.err
						.println("individual report.java - getsurveyraterstatus - "
								+ E);
			} finally {
				ConnectionBean.closeStmt(st); // Close statement
				ConnectionBean.close(con); // Close connection
	...
	public int getTotalTarget() throws SQLException {
		...
		if (departmentID != 0)
			query = query + " AND FKtargetDepartment = "
					+ departmentID;

		if (groupID != 0)
			query = query + " AND FKtargetgroup = "
					+ groupID;
		...
		try {
			...
			System.out.println("Individual.java - getTotalTarget - "
					+ ex.getMessage());
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
			ConnectionBean.closeStmt(st); // Close statement
	...
	public Vector getCompetencyRankingData(int compID) throws SQLException {
		Vector allTargets=new Vector();

		String query = "select distinct targetloginid,givenName,familyName, competencyid ,cast(avg(avgmean) as numeric(38,2)) as Result from tblavgmean ";
				query+= " inner join [user] on [user].pkuser = targetloginid where surveyid=  " + surveyID;
				query+=" and competencyid=" + compID + " group by TargetLoginID,givenName,familyName,CompetencyID order by result desc"; 
		...
		try {
			...
			while(rs.next()){
				String[] scorePair = new String[3];
				scorePair[0] = rs.getString("targetloginid");
				scorePair[1] = rs.getString("Result");
				scorePair[2] = rs.getString("familyname") + " " +  rs.getString("givenName");
				
				
				allTargets.add(scorePair);
			}

		} catch (Exception ex) {
			System.out.println("Individual.java - getCompetencyRankingData- "
					+ ex.getMessage());
			ex.printStackTrace();
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
	...
	public Vector getCP() throws Exception {
		...
		String query = "select Distinct FamilyName, GivenName, [tblavgmean].TargetLoginID, RatingTaskID, CompetencyID, ";
		query += "ROUND(AVG(tblAvgMean.AvgMean), 2) as Result";
		query += " from [tblAvgMean], [User] ,tblAssignment";
		query += " where [tblavgmean].SurveyID = " + surveyID + " and Type = " + type
				+ " and RatingTaskID = 1 and [tblavgmean].TargetLoginID = PKUser and tblassignment.round = " + round ;
		if (divisionID != 0)
			query = query + " AND FKDivision = "
					+ divisionID;
	...
	public Vector getCPScoreRanking(int compID, int surveyID) {
		...
		String query = "Select distinct FamilyName,GivenName ,round(AvgMean,2) as result ,a.targetLoginID from tblAvgMean a";
		query += " inner join Competency b on CompetencyID=PKCompetency ";
		query += " inner join [User] on a.targetLoginID = PKUser inner join tblAssignment c on a.targetLoginID=c.targetLoginID where CompetencyID ="
				+ compID + " and a.SurveyID=" + surveyID + " and TYPE ="+ type +  "and c.round= " + round;
		if (divisionID != 0)
			query = query + " AND FKDivision = "
					+ divisionID;
		...
		try {
			...
			System.err
					.println("IndividualReport.java - getCPScoreRanking - "
							+ E);
		} finally {
			ConnectionBean.closeStmt(st); // Close statement
			ConnectionBean.close(con); // Close connection
	...
	public Vector getSplitGroups(int surveyID) throws SQLException {
		Vector raters = new Vector();
		Vector temp = new Vector();
		String query = "select distinct ratercode from tblassignment where surveyID ="+surveyID + " and ratercode <> 'SELF'";
		Connection con = null;
		Statement st = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()){
				String rater = rs.getString("raterCode");
				
				temp.add(rater);
			}
			
			for(int i=0;i<temp.size();i++){
				String rater = (String) temp.elementAt(i);
				int zero = rater.indexOf("0");
				String raterCode = rater.substring(0,zero);
				
				boolean exist=false;
				for(int j=0;j<raters.size();j++){
					String curr = (String) raters.elementAt(j);
					if(raterCode.equalsIgnoreCase(curr)){
						exist=true;
					}
				}
				if(exist==false){
					raters.add(raterCode);
				}
			}
		} catch (Exception ex) {
			System.out.println("individualreport.java - getSplitGroups - "
					+ ex.getMessage());
		} finally {
			ConnectionBean.closeRset(rs); // Close ResultSet
			ConnectionBean.closeStmt(st); // Close statement
	...
	public Vector rearrangeCat(Vector v){
