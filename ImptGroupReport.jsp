...
<jsp:useBean id="ImptRptExcelGroup" class="CP_Classes.ImptGroupReport" scope="session"/>
...
function getID(form, ID, type)
{
	...
	if(ID != 0) {
		...
		var query = "ImptGroupReport.jsp?groupName=0&" + typeSelected + "=" + ID;
		form.action = query;
		form.method = "post";
		form.submit();
	} else {
...
function setSurveyImpt(form, field){
	form.action="ImptGroupReport.jsp?surveyImpt="+field.value;
	form.method="post";
	form.submit();
}
...
function confirmOpen(form)
{
	if(checkField(form)){
		...
		if(form.JobPost.value != 0 && form.surveyName.value != 0 && form.surveyNameImpt.value !=0)  {
			...
			form.action = "ImptGroupReport.jsp?open="+type+"&exGR="+exGR;
			form.submit();
			return true;	
		}else {
			alert("<%=trans.tslt("Please select the options")%>");
...
{
	form.action="ImptGroupReport.jsp?proceed="+field.value;
	form.method="post";
	form.submit();
}
...
{
	form.action="ImptGroupReport.jsp?groupName=0&div="+field1.value + "&dept=0&surveyID=" + field2.value + "&jobPost=" + field3.value;
	form.method="post";
	form.submit();
}	
...
{
	form.action="ImptGroupReport.jsp?groupName=0&div="+field1.value + "&dept=" + field2.value + "&surveyID=" + field3.value + "&jobPost=" + field4.value;
	form.method="post";
	form.submit();
}
...
{
	form.action="ImptGroupReport.jsp?groupName=" + field5.value +"&div="+field1.value + "&dept=" + field2.value + "&surveyID=" + field3.value + "&jobPost=" + field4.value;
	form.method="post";
	form.submit();
}	
...
<%	
	...
	if(request.getParameter("open") != null) {
		...
		int imptSurveyID = Integer.parseInt(request.getParameter("surveyNameImpt"));
			...
			if(deptID==0){ //department is all, group ID might be pointing more than one
				...
				if(groupID==0){
	  			...
	  			if(groupID==0){
	  				...
	  				System.out.println("deptIDList size is "+ deptIDList.size());
		...
		String file_name = "ImportanceGroupReport_"+ orgName;
		...
		System.out.println("groupIDList size is "+groupIDList.size()+" with first element is "+groupIDList.get(0));
		...
		ImptRptExcelGroup.setExGroupRanking(exGR);
		
		//Gwen Oh - 17/10/2011: Set selected users to null if it's a group report (All the users in survey are required)
		ImptRptExcelGroup.setSelectedUsers(null);
		
		ImptRptExcelGroup.Report(surveyID, imptSurveyID, designationName, groupIDList, deptIDList, divID, pkUser, file_name, type);
...
%>
  ...
  <table width="460" boQEr="0" style='font-size:10.0pt;font-family:Arial' bgcolor="#FFFFCC" border="2" bordercolor="#3399FF">
    ...
    <tr>
      <td width="100" align="right" bordercolor="#FFFFCC"><%=trans.tslt("Importance Survey")%> 
		: </td>
      <td width="10" bordercolor="#FFFFCC">&nbsp;</td>
	  <td width="230" bordercolor="#FFFFCC">
 ...
 <!-- Hide by Albert (26th July 2012): Unneccessary for MOE Group Report -->
    <tr>
      <td bordercolor="#FFFFCC">
		<p align="right">
		<%//Denise 30/12/2009 change default report to not simplified%>
		<input type="hidden" name="chkReportType" value="ON"></td>
    </tr>
    <% // Denise 16/12/2009 To add the Exclude Group Ranking box %>
    <tr>
      <td bordercolor="#FFFFCC">
		<p align="right">
		<input type="hidden" name="chkGR" value="chkGR" ></td>
    </tr>
<!-- end of commented by Albert -->    
     <tr>
      <td bordercolor="#FFFFCC">&nbsp;</td>
      <td bordercolor="#FFFFCC">&nbsp;</td>
      <td bordercolor="#FFFFCC">&nbsp;</td>
