...
function proceed(form,field){
	form.action="Report_Target_Survey.jsp?round=0&proceed="+field.value;
	form.method="post";
	form.submit();
}
function populateRound(form,field){
	form.action="Report_Target_Survey.jsp?round="+field.value;
	form.method="post";
	form.submit();
}
</script>
...
if(request.getParameter("btnPreview") != null)
{
	...
	*/
	...
	if (type==-1) Rpt7.AllTarget(SurveyID, logchk.getPKUser()); //type == -1 -> without round
	else if(type==-2) Rpt7.AllTargetWithRound(SurveyID, logchk.getPKUser(),type); //type == -2 -> all rounds
	else Rpt7.AllTargetWithRound(SurveyID, logchk.getPKUser(), type);
		
	//read the file name.
	String file_name = "List Of Targets.xls";		
	...
	response.addHeader ("Content-Disposition", "attachment;filename=\"List Of Targets.xls\"");
...
%>
<form name="Rpt_Target_Survey" action="Report_Target_Survey.jsp" method="post">
<table border="0" width="483" cellspacing="0" cellpadding="0">
	<tr>
		<td><b>
		<font face="Arial" size="2" color="#000080"><%=trans.tslt("List of Targets for Specific Survey")%></font></b></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
