	    	...
	    	{ 
	    		try
				{
		    		for(int i=0; i<chkSelect.length; i++)
					{
						...
						if (!CE_Survey.addKeyBehaviour(SurveyID,Integer.parseInt(request.getParameter("comId")),Integer.parseInt(chkSelect[i])))
...
function CompChange(form, field) 
{
	if (field.selectedIndex == 0) {
		window.location.href = "Survey_KeyBehaviourList.jsp";
	} else {
		checkData(field);
		form.action = "Survey_KeyBehaviourList.jsp?change="+data
		form.method = "post";
		form.submit();
	}
}
...
function confirmAdd(form,field, fieldKB) 
{
	checkData(field);
	if(check(fieldKB)) {
		form.action = "Survey_KeyBehaviourList.jsp?close=1&save=1"
		form.method = "post";
		form.submit();
	} 
}
...
<form name="Survey_KeyBehaviourList" method="post" action="Survey_KeyBehaviourList.jsp">
<table width="343" border="0">
    <tr>
      ...
      <%
		...
		Vector v = SC.getSurveyCompetency(CE_Survey.getSurvey_ID());
