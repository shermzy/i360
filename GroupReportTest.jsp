...
<body>
	...
	<jsp:useBean id="ExcelGroup" class="CP_Classes.GroupReport"
		scope="session" />
	<jsp:useBean id="ExcelGroupSPF" class="CP_Classes.GroupReportSPF"
		scope="session" />
		...
		<form name="GroupReport" method="post" action="GroupReport.jsp">
			...
			<%
					...
					if (request.getParameter("open") != null) {
						...
						if (request.getParameter("selOrg").equalsIgnoreCase("59")) {

						
							ExcelGroupSPF.Report(surveyID,groupIDList,deptIDList,
									divID,pkUser,file_name,type,weightedAverage);
						} else {
							System.out.println(" noSPF");
							System.out.println(" no" + clusterCompetency);

							 ExcelGroup.Report(surveyID,groupIDList,deptIDList,
									divID,pkUser,file_name,type,weightedAverage,clusterCompetency);
						}
...
-->
			<div class="row-fluid" style="padding-top: 50px">
				<div class="offset2 span2">
					<button type="button" class="btn btn-primary" name="btnOpen"
						value="<%=trans.tslt("Generate")%>"
						onclick="return confirmOpen(this.form)" >Generate</button>
				</div>
				<%
					} else {
				%>
