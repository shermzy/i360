...
function showCalendar(objDay, objMonth, objYear,Format,Title) {
...
calMonth = objMonth;
calYear = objYear;
dateField = objMonth[objMonth.selectedIndex].value + "/" + objDay[objDay.selectedIndex].text + "/" + objYear[objYear.selectedIndex].text;
calDate = new Date(dateField);
if (isNaN(calDate)) {
 calDate = new Date();
 readComplexDate(dateField);
}
...
{
	if((year >= (now.getYear() - 20)) && (year < (now.getYear() + 30)))
	{
		calDate.setFullYear(year);
	}
	else
	{
		calCtrl.year.value = now.getYear();
		calDate.setFullYear(now.getYear());
		alert("Choice of year exceeded the limit, system will set to current year");
	}
	writeCalendar();
...
//clear selection
calDay[0].selected = true;
calMonth[0].selected = true;
calYear[0].selected = true;
//stop here
ChangeOptionDays('FirstSelect'); 
ChangeOptionDays('start1'); 
ChangeOptionDays('start2'); 

var vDate, vDay, vMonth, vYear, iNextPos, iCurPos;
var now = new Date();
	iCurPos  = outDate.indexOf("/");
	vDay = outDate.substring(0, iCurPos);
	iNextPos = outDate.indexOf("/", iCurPos + 1);
	vMonth = outDate.substring(iCurPos + 1, iNextPos);
	iNextPos = outDate.indexOf("/", iCurPos + 1);
	vYear = outDate.substring(iNextPos + 1);
	
	calDay[vDay-1].selected = true;
	calMonth[vMonth-1].selected = true;
	calYear[(vYear-now.getYear()) + 20].selected = true;
	ChangeOptionDays('FirstSelect');
	ChangeOptionDays('start1'); 
	ChangeOptionDays('start2'); 
...
//function for returning how many days t
//     here are in a month including leap years
//     
...
//function to change the available days 
//     in a months
function ChangeOptionDays(Which)
{
    DaysObject = eval("document.CreateSurvey." + Which + "Day");
    MonthObject = eval("document.CreateSurvey." + Which + "Month");
    YearObject = eval("document.CreateSurvey." + Which + "Year");
