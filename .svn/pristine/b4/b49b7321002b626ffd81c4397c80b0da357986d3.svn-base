...
public class DevelopmentActivities
{
	...
	public boolean addRecord(int CompetencyID, String[] DRAStatement, String IsSystemGenerated, int companyID, int orgID, int pkUser, int userType) throws SQLException, Exception {
		...
		String sql = "Insert into tblDRA (CompetencyID, DRAStatement, DRAStatement1," +
				"DRAStatement2, DRAStatement3, DRAStatement4, DRAStatement5, IsSystemGenerated, FKCompanyID, FKOrganizationID) values (";
		//added "N" in front of dra statement by alvis on 08-Sep-09 to allow chinese support
		sql = sql + CompetencyID + ",N'" + DRAStatement[0] + "',";
		if(DRAStatement[1].trim().length() != 0)
			sql += "N'" + DRAStatement[1] +"', ";
		else
			sql +="null,";
		if(DRAStatement[2].trim().length() != 0)
			sql += "N'" + DRAStatement[2] +"',";
		else
			sql +="null,";
		if(DRAStatement[3].trim().length() != 0)
			sql += "N'" + DRAStatement[3] +"'," ;
		else
			sql +="null,";
		if(DRAStatement[4].trim().length() != 0)
			sql += "N'" + DRAStatement[4] +"',";
		else
			sql +="null,";
		if(DRAStatement[5].trim().length() != 0)
			sql += "N'" + DRAStatement[5]+"', ";
		else
			sql +="null,";
		
		sql += IsSysGenerated + ", " + companyID + ", " + orgID + ")";
		...
		String [] UserInfo = U.getUserDetail(pkUser);		
		try{
			EV.addRecord("Insert", "Development Activities", DRAStatement[0], UserInfo[2], UserInfo[11], UserInfo[10]);
		} catch(SQLException SE) {
			System.out.println(SE.getMessage());
		}
	 ...
	 *		companyID	 - the foreign key of companyID
	 *		orgID		 - the foreign key of organizationID
	 *		DRAStatement2 - foreign language version of DRA
	 *		lang - language code of foreign language : 1 Indonesian, 2 Thai, 3 Korean, 4 Traditional Chinese, 5 Simplified Chinese
	 * Returns:
	 *		a boolean that represents the success of inserting to the database.
	 */
	//Added parameter sIsSystemGenerated and related logic codes. To determine if the Develpment Activities to be added is system generated or user generated, Sebastian 29 July 2010
	public boolean addRecord(int CompetencyID, String DRAStatement, String DRAStatement2, int lang, String IsSystemGenerated, int companyID, int orgID, int pkUser, int userType) throws SQLException, Exception {
		int IsSysGenerated = 0;
		boolean bIsAdded=false;
		Connection con = null;
		Statement st = null;
		...
		String sql = null;
		if(lang == 4 || lang == 5){
			sql = "Insert into tblDRA (CompetencyID, DRAStatement, DRAStatement5, DRAStatement4, IsSystemGenerated, FKCompanyID, FKOrganizationID) values (";
			//added "N" in front of dra statement by alvis on 08-Sep-09 to allow chinese support
			sql = sql + CompetencyID + ",N'" + DRAStatement + "', N'"+ DRAStatement2 +"', N'" + DRAStatement2 +"', " + IsSysGenerated + ", " + companyID + ", " + orgID + ")";
		}else{
			sql = "Insert into tblDRA (CompetencyID, DRAStatement, DRAStatement"+lang+", IsSystemGenerated, FKCompanyID, FKOrganizationID) values (";
			//added "N" in front of dra statement by alvis on 08-Sep-09 to allow chinese support
			sql = sql + CompetencyID + ",N'" + DRAStatement + "', N'"+ DRAStatement2 +"'," + IsSysGenerated + ", " + companyID + ", " + orgID + ")";
		}
		System.out.println(sql);
		try{
			con=ConnectionBean.getConnection();
	...
	public boolean editRecord(int CompetencyID, int DRAID, String[] DRAStatement, int pkUser) throws SQLException, Exception  {
		...
		boolean bIsUpdated=false;
		//added "N" in front of dra statement by alvis on 08-Sep-09 to allow chinese support
		String sql = "Update tblDRA Set DRAStatement = N'" + DRAStatement[0] +	"',";
		if(DRAStatement[1].trim().length() != 0)
			sql += "DRAStatement1 = N'" + DRAStatement[1] +	"',";
		if(DRAStatement[2].trim().length() != 0)
			sql += "DRAStatement2 = N'" + DRAStatement[2] +	"',";
		if(DRAStatement[3].trim().length() != 0)
			sql += "DRAStatement3 = N'" + DRAStatement[3] +	"',";
		if(DRAStatement[4].trim().length() != 0)
			sql += "DRAStatement4 = N'" + DRAStatement[4] +	"',";
		if(DRAStatement[5].trim().length() != 0)
			sql += "DRAStatement5 = N'" + DRAStatement[5] +	"',";
		sql += " CompetencyID = " + CompetencyID + " where DRAID = " + DRAID;
		...
		String [] UserInfo = U.getUserDetail(pkUser);		
		try {
			EV.addRecord("Update", "Development Activities", "(" + oldStatement + ") - (" + DRAStatement[0] + ")", UserInfo[2], UserInfo[11], UserInfo[10]);
		} catch(SQLException SE) {
			System.out.println(SE.getMessage());
		}
	...
	public votblDRA getDRA(int iPkDRA) throws SQLException, Exception {
		...
		try{
			...
			if(rs.next()){
				...
				vo.setDRAStatement(rs.getString("DRAStatement"));
				for(int i = 1; i < 6; i++){
					vo.setDRAStatement(i, rs.getString("DRAStatement"+i));
				}
				vo.setFKCompanyID(rs.getInt("FKCompanyID"));
				vo.setCompetencyID(rs.getInt("CompetencyID"));
			}
