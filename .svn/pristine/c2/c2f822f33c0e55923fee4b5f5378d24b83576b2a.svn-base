...
<jsp:useBean id="Rpt6" class="CP_Classes.Report_StatusPercentage_Survey" scope="session"/>
...
function proceed(form,field){
	form.action="Report_StatusPercentage_Survey.jsp?round=0&proceed="+field.value;
	form.method="post";
	form.submit();
}
function populateRound(form,field){
	form.action="Report_StatusPercentage_Survey.jsp?round="+field.value;
	form.method="post";
	form.submit();
}
</script>
...
if(request.getParameter("btnPreview") != null)
{
	...
	*/
	...
	SimpleDateFormat dFormat = new SimpleDateFormat("dd-MM-yyyy-HHmmss");
	String temp = dFormat.format(timestamp);			
	String file_name = "List Of Surveys Status, Percentage Incomplete as of "+temp+".xls";
	System.out.println("HALOOOOOOOOOO "+file_name);
	
	if (type==-1) Rpt6.AllPercentage(SurveyID, logchk.getPKUser(),file_name); //type == -1 -> without round
	else if(type==-2) { %>
	<script>alert("Please select a round")</script>
	<%
	}
	else Rpt6.AllPercentageWithRound(SurveyID, logchk.getPKUser(), type,file_name);
...
%>
<form name="Rpt_StatusPercentage_Survey" action="Report_StatusPercentage_Survey.jsp" method="post">
<table border="0" width="49%" cellspacing="0" cellpadding="0">
	<tr>
		<td> <font size="2">
   
    	<b>
		<font face="Arial" size="2" color="#000080"><%=trans.tslt("List of Surveys' Status - Percentage Incomplete")%></font></b></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
...
<%
	...
	<tr>
		...
		<option value="-2" selected><%=trans.tslt("Please select one")%></option>
	<%
