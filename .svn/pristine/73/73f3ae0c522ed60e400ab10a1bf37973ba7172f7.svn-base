...
function DeleteAway(form,field)
{
	if(check(field))
	{
		...
		{
			form.action="SurveyList_Create.jsp?del=1";
			form.method="post";
			form.submit();	
		}
	}
...
function confirmAdd(form) {
		var myWindow=window.open('Survey_Setup.jsp?copyJobCat=' + document.SurveyList.selOrg.value ,'','scrollbars=yes, width=600, height=300');
		myWindow.moveTo(x,y);
}
