...
<% 	}
  ...
  <form name="AdditionalAnswers" action="AdditionalAnswers.jsp" method="post" accept-charset="ISO-8859-1">
...
for(int i=0;i<v.size();i++)
{
	...
	if(va.size()>0||QuestionBean.getAnswerCount(i)>0)
	{
		...
		for(int x=0; x<QuestionBean.getAnswerCount(i);x++)
		{
			...
			if(x<va.size()) //check if the answer is saved already, retrieve the answer from db
			{
				...
				out.println("<textarea rows=2 cols=65 name=qid"+aq.getAddQnId()+"ans"+x+" id=qid"+aq.getAddQnId()+"ans"+x+">"+aqa.getAnswer()+"</textarea>");
			}
			else
			{
				out.println("<input type=\"hidden\" name=qid"+aq.getAddQnId()+"aid"+x+" id=qid"+aq.getAddQnId()+"aid"+x+" value=0 />");
				out.println("<textarea rows=2 cols=65 name=qid"+aq.getAddQnId()+"ans"+x+" id=qid"+aq.getAddQnId()+"ans"+x+"></textarea>");
			}
	...
	else
	{
		...
		out.println("<textarea rows=2 cols=65 name=qid"+aq.getAddQnId()+"ans0 id=qid"+aq.getAddQnId()+"ans0></textarea>");
		out.println("</td>");
		out.println("</tr>");
